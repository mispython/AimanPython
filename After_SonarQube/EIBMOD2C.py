...
PBB_OUTPUT_PATH   = BASE_DIR / "pbb/odlis2.cold.txt"              # SAP.PBB.ODLIS2.COLD
...
PIBB_OUTPUT_PATH  = BASE_DIR / "pibb/odlis2.cold.txt"             # SAP.PIBB.ODLIS2.COLD
...
def emit_total_block(emit_fn, check_page_break_fn, label: str, code: str, balance_sum: float, content_pos54: int):
    """Emit separator and total lines for subtotal/grand total sections."""
    check_page_break_fn(3)
    emit_fn(asa_newline(), build_separator_line(15, 52))
    total_line = ' ' * 14 + label + code
    total_line = total_line.ljust(content_pos54) + fmt_numeric(balance_sum, 13, 2)
    emit_fn(asa_newline(), total_line)
    emit_fn(asa_newline(), build_separator_line(15, 52))


def process_fisspurp_group(fp_df: pl.DataFrame, emit_fn, check_page_break_fn) -> float:
    """Emit rows for one FISSPURP group and return BALANCE subtotal."""
    fp_balance_sum = 0.0
    for row in fp_df.to_dicts():
        check_page_break_fn(1)
        emit_fn(asa_newline(), format_data_row(row))
        fp_balance_sum += safe_float(row.get('BALANCE', 0))
    return fp_balance_sum
...
def generate_report(
    ...
    lines = []      # list of full fixed-width lines (ASA char + content)
    ...
    def emit_page_header():
        ...
        emit(asa_newline(), '')
        page_num += 1
        line_count = 9  # reset after header block

    # Emit first page header
    emit_page_header()

    branches = df_sorted['BRANCH'].unique(maintain_order=True).to_list()
    # Position 54 (1-based) in content = index 53 (0-based)
    content_pos54 = 53
    ...
    for branch in branches:
        ...
        for fisspurp in fisspurps:
            ...
            fp_balance_sum = process_fisspurp_group(fp_df, emit, check_page_break)
...
def run_pbb():
    ...
    # DATA ODRAFT2: SET ODLC.ODRAF2&REPTMON
    odraf2_path = PBB_ODLC_PATH / f"odraf2{reptmon}.parquet"
    con = duckdb.connect()
    odraft2_df = con.execute(
        f"SELECT * FROM read_parquet('{odraf2_path}')"
    ).pl()
    con.close()
    ...
    title3 = (
        'OD LISTING BY CONSTRUCTION (SECTCODE 5001-5999) AND'
        '                                       REPORT DATE: ' + rdate
    )
    title4 = 'REAL ESTATE (SECTCODE 8310) FOR NON-IND.CUSTOMER'

    generate_report(
        odraft_df=odraft2_df,
        title1=title1,
        title2=title2,
        title3=title3,
        title4=title4,
...
def run_pibb():
    ...
    # DATA ODRAFT2: SET ODLCI.ODRAF2&REPTMON
    odraf2_path = PIBB_ODLCI_PATH / f"odraf2{reptmon}.parquet"
    con = duckdb.connect()
    odraft2_df = con.execute(
        f"SELECT * FROM read_parquet('{odraf2_path}')"
    ).pl()
