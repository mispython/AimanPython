...
CCOLLAT_CUTOFF_DATE = date(2018, 7, 1)


def parse_collateral_eff_date(coll_eff):
    if coll_eff in (None, 0):
        return None

    try:
        eff_date_str = str(int(coll_eff)).zfill(11)[:8]
        return datetime.strptime(eff_date_str, "%m%d%Y").date()
    except (TypeError, ValueError):
        return None


def parse_collateral_start_date(coll_start):
    if coll_start in (None, 0):
        return None

    try:
        start_date_str = str(int(coll_start)).zfill(8)
    except (TypeError, ValueError):
        return None

    if start_date_str == "00000000":
        return None
...
def should_skip_ccollat_row(eff_dx, start_dx):
    return eff_dx is not None and eff_dx >= CCOLLAT_CUTOFF_DATE and start_dx is None


def process_ccollat(df):
    result = []
    for row in df.to_dicts():
        eff_dx = parse_collateral_eff_date(row.get("COLLATERAL_EFF_DT"))
        start_dx = parse_collateral_start_date(row.get("COLLATERAL_START_DT"))

        if should_skip_ccollat_row(eff_dx, start_dx):
            continue

        row["_EFF_DX"] = eff_dx
        row["_START_DX"] = start_dx
        result.append(row)
...
collater_df = pl.concat([
    loan_df.select(loan_df.columns),
    ...
    btrl_df.select(btrl_df.columns)
], how="diagonal")
...
def get_collater_code(cclassc, cissuer):
    ...
    if ciss.startswith(("KLM", "UT")):
        code = '23'
    return code

def build_collater_record(r, ficode, apcode, collref, collater_code, collval):
...
for r in rows:
    ...
    collater_rec = build_collater_record(r, ficode, apcode, collref, collater_code, collval)
...
def val_date_components(valdt_str, reptday_int, bkmth, bkyyr, bkday):
    ...
    if reptday_int in (8, 15, 22) and vtmm > bkmth and vtyy >= bkyyr:
        vtmm = bkmth
        vtyy = bkyyr
        vtdd = bkday
