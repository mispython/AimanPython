...
# Constants
ACELIMIT = 5000
DEFAULT_TIER = 999999999

TIER_THRESHOLDS = {
    177: [5000],
    200: [2500, 5000, 10000, 30000, 50000, 75000],
    201: [2500, 5000, 10000, 30000, 50000, 75000],
    202: [500, 2000, 5000, 10000, 30000, 50000, 75000],
    203: [2000, 5000, 10000, 30000, 50000, 75000],
    212: [5000, 10000, 20000, 30000, 50000],
    213: [5000, 10000, 30000, 50000, 75000],
    214: [1000, 5000, 25000, 50000],
    215: [50000],
}
...
def assign_tier(product, curbal):
    ...
    if product in [150, 151, 152, 181]:
        mapped_product = 150
        if curbal > ACELIMIT:
            return mapped_product, DEFAULT_TIER, curbal - ACELIMIT
        return mapped_product, 5000, 0

    if product == 204:
        return product, DEFAULT_TIER, curbal

    thresholds = TIER_THRESHOLDS.get(product)
    if thresholds is None:
        return product, 0, 0

    for threshold in thresholds:
        if curbal < threshold:
            return product, threshold, curbal
