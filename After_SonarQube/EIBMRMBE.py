...
VALUE_COLUMNS = [
    'TOTAL', 'TOTAL_A', 'BNKINSTI', 'CORPRETAIL', 'CORPORATE',
    'RETAIL', 'TOTAL_F', 'FORBNKINSTI', 'FORNONBNK'
]


def _empty_table_row(prefix=' |'):
    return prefix + (' ' * 20 + '|') * 10


def _separator_row(char='-'):
    return ' |' + '|'.join([char * 29] + [char * 20] * 9) + '|'


def _format_data_line(label, values):
    return ' |' + f"{label:<29}" + '|' + '|'.join(format_amount(values[col]) for col in VALUE_COLUMNS) + '|'


def _get_numeric_values(row):
    return {col: (row[col] if row[col] is not None else 0.0) for col in VALUE_COLUMNS}


def _append_report_header(lines):
    lines.extend([
        ' TABLE 1 - STATEMENT OF FINANCIAL POSITION',
        ' ',
        ' ' + '-' * 219,
        ' |' + ' ' * 29 + '|' + ' ' * 20 + '|' + ' ' * 41 + 'RESIDENT' + ' ' * 62 + '|' + ' ' * 20 + 'NON RESIDENT' + ' ' * 41 + '|',
        ' |' + ' ' * 29 + '|' + ' ' * 20 + '|' + '-' * 104 + '|' + '-' * 62 + '|',
        ' |' + ' ' * 29 + '|' + ' ' * 20 + '|' + ' ' * 20 + '|' + ' ' * 20 + '|' + ' ' * 62 + '|' + ' ' * 20 + '| FOREIGN  ' + ' ' * 20 + '| FOREIGN  ' + '|',
        ' |' + ' ' * 29 + '|' + ' ' * 20 + '|' + ' ' * 20 + '|' + ' ' * 20 + '|   NON-BANK ENTITY  ' + ' ' * 41 + '| TOTAL(F) ' + ' ' * 20 + '| BANKING ' + ' ' * 20 + '| NON-BANK ' + '|',
        ' |' + ' ' * 29 + '| TOTAL=TOTAL(A)' + ' ' * 6 + '| TOTAL(A) ' + ' ' * 9 + '| BANKING ' + ' ' * 10 + '|' + ' ' * 62 + '| =(G)+(H)' + ' ' * 10 + '| INSTITUTION(G)' + ' ' * 6 + '| ENTITY (H)' + ' ' * 8 + '|',
        ' | ITEM ' + ' ' * 22 + '| + TOTAL(F)' + ' ' * 10 + '| = (B) + (C)' + ' ' * 7 + '| INSTITUTION(B)' + ' ' * 6 + '|' + '-' * 62 + '|' + '-' * 20 + '|' + '-' * 20 + '|' + '-' * 20 + '|',
        ' |' + ' ' * 29 + '|' + ' ' * 20 + '|' + ' ' * 20 + '|' + ' ' * 20 + '| TOTAL(C)' + ' ' * 11 + '| CORPOTATE(D)' + ' ' * 8 + '| RETAIL(E)' + ' ' * 9 + '|' + ' ' * 20 + '|' + ' ' * 20 + '|' + ' ' * 20 + '|',
        ' |' + ' ' * 29 + '|' + ' ' * 20 + '|' + ' ' * 20 + '|' + ' ' * 20 + '| = (D) + (E)' + ' ' * 9 + '|' + ' ' * 20 + '|' + ' ' * 20 + '|' + ' ' * 20 + '|' + ' ' * 20 + '|' + ' ' * 20 + '|',
        _separator_row(),
    ])


def _handle_btac_fcybt(lines, values):
    lines.extend([
        ' | TOTAL ASSETS' + ' ' * 16 + _empty_table_row(prefix='|'),
        _empty_table_row(),
        ' | - LOANS/FINANCING AND' + ' ' * 7 + _empty_table_row(prefix='|'),
        ' |   RECEIVABLES' + ' ' * 15 + _empty_table_row(prefix='|'),
        _format_data_line('   (NET OF PROVISION)', values),
        ' ' + '-' * 219,
    ])


def _handle_dpac_fcyfd(lines, values):
    lines.extend([
        ' | DEPOSITS ACCEPTED' + ' ' * 11 + _empty_table_row(prefix='|'),
        _empty_table_row(),
        _format_data_line('    - FX FIXED DEPOSITS', values),
    ])


def _handle_dpac_fcydd(lines, values):
    lines.extend([
        _format_data_line('    - FX DEMAND DEPOSITS', values),
        _separator_row(),
    ])


def _handle_total(lines, values):
    lines.extend([
        _format_data_line(' TOTAL', values),
        ' ' + '-' * 219,
    ])
...
def generate_report(sumtable1, output_file):
    ...
    handlers = {
        ('BTAC', 'FCYBT'): _handle_btac_fcybt,
        ('DPAC', 'FCYFD'): _handle_dpac_fcyfd,
        ('DPAC', 'FCYDD'): _handle_dpac_fcydd,
        ('TOTAL', 'TOTAL'): _handle_total,
    }
    ...
    for idx, row in enumerate(sumtable1.iter_rows(named=True), 1):
        ...
        handler = handlers.get((row['CATEGORY'], row['GROUP']))
        if handler:
            handler(lines, _get_numeric_values(row))
