...
def get_numeric_value(row, key):
    """Safely get numeric value from a row, defaulting to zero."""
    return row.get(key, 0) or 0


CGC_FIELD_MAP = {
    'REPAYMENT': ('NOREP', 'REPAID', 'NOREPX', 'REPAIDX'),
    'DISBURSEMENT': ('NODIS', 'DISBURSE', 'NODISX', 'DISBURSEX'),
    'OUTSTANDING': ('NOBAL', 'BALANCE', 'NOBALX', 'BALANCEX')
}
...
def format_cgc_row(row):
    ...
    type_val = row['TYPE']
    field_names = CGC_FIELD_MAP.get(type_val)

    if field_names:
        noacct_key, amount_key, noacctx_key, amountx_key = field_names
        noacct = get_numeric_value(row, noacct_key)
        amount = get_numeric_value(row, amount_key)
        noacctx = get_numeric_value(row, noacctx_key)
        amountx = get_numeric_value(row, amountx_key)
    else:
        noacct, amount, noacctx, amountx = 0, 0, 0, 0
...
def format_il_row(row):
    ...
    typx = row['TYPX']
    il_bucket_index = {'A': 1, 'B': 2, 'C': 3, 'D': 4}.get(typx)

    if il_bucket_index is None:
        noacct, amount, noacctx, amountx = 0, 0, 0, 0
    else:
        noacct = get_numeric_value(row, f'NO{il_bucket_index}')
        amount = get_numeric_value(row, 'BALANCE')
        noacctx = get_numeric_value(row, f'NO{il_bucket_index}X')
        amountx = get_numeric_value(row, 'BALANCEX')
...
with open(OUTPUT_FILE, 'w') as f:
    ...
    f.write(BLANK_LINE * SECTION_SPACING_LINES)
