...
def is_missing_value(val):
    """Return True when val is None or a NaN-like numeric value."""
    if val is None:
        return True
    try:
...
def coalesce(val, default=0.0):
    """Return default when val is missing; otherwise return val."""
    if is_missing_value(val):
        return default
    return val

def prefer_joined_column(df: pl.DataFrame, column: str, suffix: str) -> pl.DataFrame:
    """Prefer a joined suffix column when present, then drop the suffix column."""
    joined_column = f"{column}{suffix}"
    if joined_column not in df.columns:
        return df

    fallback = pl.col(column) if column in df.columns else pl.lit(None)
    return df.with_columns(
        pl.when(pl.col(joined_column).is_not_null())
          .then(pl.col(joined_column))
          .otherwise(fallback)
          .alias(column)
    ).drop(joined_column)


def calculate_floamt(row: dict, noofday: int, dayyr: int) -> float:
    """Compute FLOAMT for a DPBAL row."""
    ...
    return ((avgbal * (float_r / 100)) * (noofday / dayyr)) - intrstpd


def load_name_lookup() -> pl.DataFrame:
    """Load the account-name lookup table sorted by account number."""
    con = duckdb.connect()
    name_df = con.execute(
...
def resolve_name_columns(df: pl.DataFrame) -> pl.DataFrame:
    """Resolve joined name columns back into their base column names."""
    for column in ['nmabbr', 'custname']:
        df = prefer_joined_column(df, column, '_name')
    return df
...
def process_main(rv: dict, rate: pl.DataFrame) -> tuple:
    ...
    rate_sel = rate.select(rate.columns)
    ...
    for row in rows_data:
        ...
        out = dict.fromkeys(cnt_cos_cols)
...
def process_depbal(rv: dict, rate: pl.DataFrame) -> tuple:
    ...
    dpbal = prefer_joined_column(dpbal, 'float', '_rate')

    dayyr = 366 if (reptyear % 4 == 0) else 365
    rows = dpbal.to_dicts()
    floams = [calculate_floamt(row, noofday, dayyr) for row in rows]
    ...
    dpbal = resolve_name_columns(dpbal)
...
def write_costxt01(totsum: pl.DataFrame, output_path: str):
    ...
    with open(output_path, 'w', encoding='ascii', errors='replace') as f:
        # Header row 1
        header_row_1 = ';'.join([
            '', '', 'SETUP/DIRECT COST', '', '', 'TOTAL OPERATING',
            'INCOME(RM)', '', '', 'PROFIT/', ''
        ])
        f.write(f"{header_row_1}\n")
        # Header row 2
...
def write_print_report(totsum: pl.DataFrame, rv: dict, output_path: str):
    ...
    def newpage(f, rate_row):
        ...
        place_at(line, 34,  TRX_VOL_LABEL)
        place_at(line, 52,  COST_LABEL)
        place_at(line, 59,  TRX_VOL_LABEL)
        place_at(line, 77,  COST_LABEL)
        place_at(line, 84,  TRX_VOL_LABEL)
        place_at(line, 102, COST_LABEL)
        place_at(line, 109, TRX_VOL_LABEL)
        place_at(line, 127, COST_LABEL)
        f.write(' ' + finalize_line(line, PAGE_WIDTH) + '\n')
