#!/usr/bin/env python3
"""
File Name: EIBWPSCH.py
Weekly Loan Schedule Payment Extraction to Warehouse
...
# ============================================================================
# Input paths
INPUT_LN_REPTDATE = "input/EIBWPSCH/ln_reptdate.parquet"
INPUT_LN_LNNOTE = "input/EIBWPSCH/ln_lnnote.parquet"
INPUT_PAYFILE = "input/EIBWPSCH/paysfile.dat"  # Binary file with packed decimal

# Output paths
OUTPUT_LNPAY_WEEK = "/mnt/user-data/outputs/lnpay_week{week}.parquet"  # e.g., lnpay_week01.parquet
OUTPUT_LNPYWFTP = "/mnt/user-data/outputs/lnpywftp.dat"  # Transport file (binary)
OUTPUT_PRINT_REPORT = "/mnt/user-data/outputs/lnpay_print_report.txt"  # Print output
    ...
    try:
        with open(filepath, 'rb') as f:
            while True:
                ...
                # Maximum position is @061 + 6 bytes = 66 bytes minimum
                record = f.read(100)  # Read more to be safe
...
def _extract_effdate_parts(effdat_packed):
    """Extract year, month, and day from the packed effective date value."""
...
def _normalize_effdate_day(year, month, day):
    """Normalize special day values to the last valid day for the month."""
    if day not in {29, 30, 31, 99}:
        return day

    if month == 2:
        is_leap_year = year % 4 == 0
        return 29 if day in {29, 30, 31, 99} and is_leap_year else 28
...
print(f"Loan note records loaded: {len(lnnote):,}")
...
# Merge and filter (keep only records in lnnote, and where NOPAY > 0)
lnpay_week = con.execute("""
    SELECT 
        n.ACCTNO,
        n.NOTENO,
        p.PAYTYPE,
...
print(f"Final payment schedule records: {len(lnpay_week):,}")
...
output_parquet = OUTPUT_LNPAY_WEEK.format(week=NOWK)
Path(output_parquet).parent.mkdir(parents=True, exist_ok=True)

con.register('lnpay_week', lnpay_week)
con.execute(f"COPY lnpay_week TO '{output_parquet}' (FORMAT PARQUET)")
...
with open(OUTPUT_PRINT_REPORT, 'w') as f:
    ...
    f.write('1')  # New page
    f.write(f"{'Weekly Loan Payment Schedule':^132}\n")
    f.write(' ')
    ...
    obs_count = 0
    for idx, row in lnpay_week.iterrows():
    ...
    f.write(' ')
    f.write(f"Total observations: {len(lnpay_week):,}\n")
...
Path(OUTPUT_LNPYWFTP).parent.mkdir(parents=True, exist_ok=True)
...
with open(OUTPUT_LNPYWFTP, 'wb') as dst:
    # Write a simple header to identify this as our transport format
    header = b'LNPAY_TRANSPORT_V1'
...
print(f"Transport file created: {OUTPUT_LNPYWFTP}")
...
if len(lnpay_week) > 0:
    ...
    print(f"  Total Payment Amount: ${stats[2]:,.2f}" if stats[2] else "  Total Payment Amount: $0.00")
    print(f"  Average Payment: ${stats[3]:,.2f}" if stats[3] else "  Average Payment: $0.00")
    if stats[4]:
