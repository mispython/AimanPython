...
def _write_page_header(fh, rdate: str) -> int:
    """Write the report page header and return the number of lines consumed."""
    fh.write(f"1{'REPORT ID : EIMNOSTE':^132}\n")                                         # '1' = new page
    fh.write(f" {'REPORT ON FOREIGN EXCHANGE TRANSACTION AS AT ' + rdate:^132}\n")        # ' ' = single space
    fh.write(" \n")                                                                         # blank line
    fh.write(f" {'CURRENCY':<8} {'AGENT':<5} {'NAME':<30} {'TRANSACTION':<20} "
             f"{'DEBIT(D)/':<9} {'NO OF':<8} {'FOREIGN':<17} {'RM':<17}\n")
    fh.write(f" {'CODE':<8} {'NO':<5} {'':<30} {'DESCRIPTION':<20} "
             f"{'CREDIT(C)':<9} {'TRANS':<8} {'AMOUNT':<17} {'AMOUNT':<17}\n")
    fh.write(f" {'':<8} {'':<5} {'':<30} {'':<20} {'':<9} {'ACTION':<8} {'':<17} {'':<17}\n")
    fh.write(f" {'-' * 132}\n")
    return 7   # header occupies 7 lines
...
with open(OUTPUT_REPORT, "w") as fh:
    ...
    for i, row in enumerate(rows):
        ...
        if current_agentno is not None and row["AGENTNO"] != current_agentno:
            fh.write(f" {' ' * 94}{'-' * 36}\n")
            fh.write(f" {' ' * 94}{agent_forcur:>17,.2f} {agent_rmcur:>17,.2f}\n")
            fh.write(f" {' ' * 94}{'-' * 36}\n")
            fh.write(" \n")
            line_count   += 4
            agent_forcur  = 0.0
        ...
        detail += f"{name:<30} {trdesc:<20} {sign:<9} {notran:>8,} {forcur:>17,.2f} {rmcur:>17,.2f}\n"
        fh.write(detail)
        line_count += 1
    ...
    # ── grand total ───────────────────────────────────────────────────────
    fh.write(" \n")
    fh.write(f" {' ' * 86}{'TOTAL':>7} {total_forcur:>17,.2f} {total_rmcur:>17,.2f}\n")
    fh.write(f" {' ' * 94}{'=' * 36}\n")
