...
# Mainframe fixed-width record length (RECFM=FB LRECL=1150).
# The widest field ends at byte position 1150 (1-based: @1101 + 50 chars),
# so each physical record is exactly LRECL bytes.  Reading fixed-length
# chunks avoids the rstrip(\r\n) trap where EBCDIC 0x0D mid-record would
# corrupt byte offsets when iterating line-by-line.
LRECL = 1150
...
def decode_packed_decimal(raw: bytes, decimal_places: int = 2) -> float:
    ...
    Fix: sign_nibble is initialised to 0xF (unsigned positive) before the
    loop so it is always defined even when raw is empty or a single byte.
    """
    if not raw:
        return 0.0
...
#
# KEY FIX: The file is read as a binary stream in fixed-length chunks of
# exactly LRECL bytes (RECFM=FB).  The previous line-by-line approach called
# rstrip(b"\r\n") which silently stripped any EBCDIC byte whose value equals
# 0x0D (carriage-return in ASCII) even when it appeared in the middle of a
# record, corrupting every byte offset after that point and producing
# garbled / "alien" output.  Reading raw fixed-length records sidesteps this
# entirely because no newline stripping is needed or performed.
# =============================================================================
#
...
def parse_ecp_flat_file(filepath: Path, reptdate_val: date) -> pl.DataFrame:
    ...
    The file is opened in binary mode and consumed as fixed-length records
    of exactly LRECL bytes (RECFM=FB).  No newline stripping is performed,
    which means EBCDIC bytes whose numeric value coincides with ASCII control
    characters (0x0D, 0x0A, etc.) are preserved intact inside each record.
    """
    rows = []

    with open(filepath, "rb") as fh:
        while True:
            record_bytes = fh.read(LRECL)
            if not record_bytes:
                break           # end of file
            if len(record_bytes) < LRECL:
                # Partial final record — pad with EBCDIC spaces (0x40) to
                # avoid short-read errors on the last physical block.
                record_bytes = record_bytes + b"\x40" * (LRECL - len(record_bytes))

            record: dict = {}
            for (name, start1, length, ftype) in FIELD_SPECS:
                s = start1 - 1          # 0-based start
                e = s + length
                # Slice directly from the fixed-length record; no padding
                # needed because the record is guaranteed to be LRECL bytes.
...
#
# TRN and CIS subsets are written as pipe-delimited text files (.txt).
# TRANDATE is formatted as YYYYMMDD to match the SAS YYMMDD10. format output.
# =============================================================================
def write_txt(df: pl.DataFrame, filepath: Path) -> None:
    ...
    """
    # Cast Date columns to string in ISO format before writing
    cast_exprs = [
