...
# MIS.DYIBUW<MM> parquet - built dynamically from reptmon
# e.g. MIS.DYIBUW06 -> DYIBUW06.parquet

# Output report file
REPORT_FILE = os.path.join(OUTPUT_DIR, 'DIIMIS7W.txt')
...
def main():
    ...
    # ------------------------------------------------------------------
    # Step 2 : Read MIS.DYIBUW<REPTMON> parquet
    # (differs from DIIMIS7O which reads DYIBUO, and DIIMIS7P which reads DYIBUP)
    # ------------------------------------------------------------------
    dyibuw_parquet = os.path.join(INPUT_DIR, f'DYIBUW{reptmon}.parquet')

    raw_df = con.execute(
        f"SELECT * FROM read_parquet('{dyibuw_parquet}')"
    ).pl()
    ...
    with open(REPORT_FILE, 'w', encoding='utf-8') as f:
        ...
        # ==============================================================
        # OLD REPORT SECTION
        # TITLE1 'REPORT ID : DIIMIS7W (OLD)'
        # ==============================================================
        writer.set_titles(
            'REPORT ID : DIIMIS7W (OLD)',
            'PUBLIC ISLAMIC BANK BERHAD - IBU',
            'NEW PBB & PFB ISTISMAR FD ACCOUNT AFTER PRIVATISATION',
        ...
        # ==============================================================
        # NEW REPORT SECTION
        # TITLE1 'REPORT ID : DIIMIS7W (NEW)'
        # ==============================================================
        writer.new_page()
        writer.set_titles(
            'REPORT ID : DIIMIS7W (NEW)',
            'PUBLIC ISLAMIC BANK BERHAD - IBU',
            'NEW PBB & PFB ISTISMAR FD ACCOUNT AFTER PRIVATISATION',
            f'(16-AUG-11) AS AT {rdate}',
        )
