...
Purpose  : NLF Report - Contractual Run-offs as of End of Month (Run-Off Format).
           ...
           Structural differences from EIIDNLF0 (the monthly equivalent):
             1. FCY_PRODUCTS set contains 27 products (full list including
                805-806, 807-817) vs 15 products in EIIDNLF0.
             2. ADDEIR ITEM derivation: WHEN ('FL') maps to '211' only;
                'HL' products fall through to '219'. In EIIDNLF0 the ADDEIR
                step uses WHEN ('FL','HL') -> '211'. This difference is
                faithfully reproduced per the SAS source.
             3. Program name and output path prefix reflect EIBDNLF0.

           Dependencies:
             %INC PGM(PBBLNFMT) ->
               LIQPFMT format is used via PUT(PRODUCT, LIQPFMT.) to classify
                products as 'FL', 'HL', 'RC', 'OD' etc.
               LIQPFMT is NOT present in the attached PBBLNFMT.py.
...
# BNM.REPTDATE output (also used within same run) -> SAP.PBB.NLF.LN.DAILY
BNM_REPTDATE_PATH = OUTPUT_DIR / "bnm_reptdate.parquet"

# BNM.NOTE final output -> SAP.PBB.NLF.LN.DAILY
BNM_NOTE_OUTPUT = OUTPUT_DIR / "bnm_note.parquet"

# %LET FCY - full 27-product list as declared in EIBDNLF0 SAS source.
# Note: EIIDNLF0 carried only 15 products; EIBDNLF0 includes the
# complete set 800-817, 851-860.
...
# program's branching logic which expects values: 'HL', 'FL', 'RC'.
#
...
#   All others -> 'FL'
# Products 225,226 are treated as HL per the filter condition and PRDFMT
# patterns in PBBLNFMT.
# ============================================================================
...
# ============================================================================
# ITEM derivation helpers
# IF CUSTCD IN ('77','78','95','96') THEN CUST = '08'; ELSE CUST = '09'
#
# Main DATA NOTE step (same as EIIDNLF0):
#   individual (08): HL -> '214', else -> '219'
#   other      (09): FL/HL -> '211', RC -> '212', else -> '219'
#
# ADDEIR step (differs from EIIDNLF0 - HL is NOT included in '211' branch):
#   individual (08): HL -> '214', else -> '219'
#   other      (09): FL -> '211', RC -> '212', else -> '219'
#                    Note: 'HL' falls through to '219' here.
#
# PRODUCT = 100 -> ITEM = '212' in both steps (hardcoded - ABBA HL Financing)
# ============================================================================

def derive_item(prod_type: str, cust: str, product: int) -> str:
    """
    Derive ITEM code for the main DATA NOTE step.
    individual (08): HL -> '214', else -> '219'
    other      (09): FL/HL -> '211', RC -> '212', else -> '219'
    PRODUCT=100 -> always '212' (ABBA HL Financing, hardcoded).
    """
    if cust == '08':
...
def derive_item_eir(prod_type: str, cust: str, product: int) -> str:
    """
    Derive ITEM code for the ADDEIR step.
    Differs from derive_item(): WHEN ('FL') only -> '211'; 'HL' falls to '219'.
    individual (08): HL -> '214', else -> '219'
    other      (09): FL -> '211', RC -> '212', else -> '219'
    PRODUCT=100 -> always '212' (ABBA HL Financing, hardcoded).
    """
...
def process_loans(df: pl.DataFrame, env: dict, runoffdt: date) -> pl.DataFrame:
    ...
    # RUNOFFDT components for %REMMTH base (SAS uses RUNOFFDT not REPTDATE here)
    ro_yr  = runoffdt.year
    ro_mth = runoffdt.month
    ro_day = runoffdt.day

    # DAYS base is REPTDATE (DAYS = REPTDATE - BLDATE per SAS)
    output_rows = []
    ...
    for r in df.to_dicts():
        ...
        # ----------------------------------------------------------------
        # OD accounts: fixed REMMTH = 0.1, single output row
        # IF ACCTYPE = 'OD'
        # ----------------------------------------------------------------
        if acctype == 'OD':
            remmth = 0.1
...
#
# Note: ITEM derivation here differs from the main DATA NOTE step.
#       WHEN ('FL') -> '211' only; 'HL' is NOT included and falls to '219'.
#       See derive_item_eir() for the precise mapping.
# ============================================================================
...
def build_eir_adj(path: Path) -> pl.DataFrame:
    ...
    Uses derive_item_eir() which maps FL->211 only (not FL/HL as in EIIDNLF0).
    """
    con = duckdb.connect()
    ...
    for r in df.to_dicts():
        ...
        product   = int(r.get("PRODUCT", 0) or 0)
        prod_type = liqpfmt(product)
        item      = derive_item_eir(prod_type, cust, product)
...
def main() -> None:
    """
    Main entry point replicating EIBDNLF0 SAS program logic:
