...
"""
Program: EIBCAP43
Purpose: Generate PBB Movement of CAP by Category report
"""
...
OUTPUT1_PATH = os.path.join(OUTPUT_DIR, "EIBCAP43.txt")
...
# TBL3 = PBB MOVEMENT OF CAP BY CATEGORY AS AT
TBL3 = "PBB MOVEMENT OF CAP BY CATEGORY AS AT"

# ─────────────────────────────────────────────
# Load NPL.CAP{REPTMON}{REPTYEAR}
# Dependency: EIBCAP42.py → produces npl/cap{REPTMON}{REPTYEAR}.parquet
# ─────────────────────────────────────────────
cap_path = os.path.join(NPL_DIR, f"cap{REPTMON}{REPTYEAR}.parquet")
cap_df   = con.execute(f"SELECT * FROM read_parquet('{cap_path}')").pl()
...
RTS = 40  # row title size
COL_W = 20

def hdr_line() -> str:
    cats_hdr = f"{'CATEGORY':^20}{'BRANCH':^20}"
    vals_hdr = "".join(f"{VAR_LABELS[c]:>{COL_W}}" for c in VAR_COLS)
    return f"{'':>{RTS}}{vals_hdr}"

def sep_line(char="-") -> str:
    total = RTS + COL_W * len(VAR_COLS)
    return char * total
...
# Page header
emit("1", f"{TBL3} {DATE_STR}")
emit(" ", "")

# Column headers
emit(" ", f"{'CATEGORY':^20}{'BRANCH':^20}" + "".join(f"{VAR_LABELS[c]:>{COL_W}}" for c in VAR_COLS))
emit(" ", sep_line())

# Iterate by NO → CATEGORY → BRANCH1
grand_totals = {c: 0.0 for c in VAR_COLS}

for no_val in sorted(bycat_df["NO"].unique().to_list()):
    cat_block = bycat_df.filter(pl.col("NO") == no_val)
