# !/usr/bin/env python3
"""
Program Name: EIIWCC5L
Purpose: Process CCRIS collateral data from multiple sources (including ELDS/UCOLL records)
            for PIBB institution. Outputs fixed-width text files categorized by collateral type:
         ...
         Each collateral type produces two output files: primary and ELDS (UCOLL='Y').
         Differences from X_EIBWCC5L: uses BTRD.IMAST source, PIBB DSN paths,
            different CCLASSC groupings (no '147' in SET_CFINASST, '111' in CCONCESS,
            131-141 in COTHVEHI not CMTORVEH), and no CCLASSC='147' override in CFINASST.
"""
...
# Input parquet files
BTRD_IMAST_PARQUET       = os.path.join(BASE_INPUT_DIR, "btrd_imast.parquet")           # BTRD.IMAST&REPTDAY&REPTMON
...
def fmt_num(val, width):
    """Format numeric value right-justified, space-padded integer (SAS w. format)."""
    try:
...
def fmt_str(val, width):
    """Format string left-justified, space-padded (SAS $w. format)."""
    s = safe_str(val)
...
def make_rec(length):
    """Create a blank fixed-width record buffer."""
    return [" "] * length
...
def write_fixed(rec, length):
    """Return newline-terminated fixed-width string from record buffer."""
    return "".join(rec[:length]) + "\n"
...
# =============================================================================
# STEP 2: READ AND MERGE BTRD.IMAST + BNM.LNACCT → BTRL
# =============================================================================
btrd_df = con.execute(
    f"SELECT * FROM read_parquet('{BTRD_IMAST_PARQUET}') ORDER BY ACCTNO"
).pl()
...
def process_ccollat(df: pl.DataFrame) -> pl.DataFrame:
    ...
    for r in rows:
        ...
        if coll_eff is not None and coll_eff != 0:
            try:
                s8 = str(int(coll_eff)).zfill(11)[:8]   # Z11. first 8 chars → MMDDYYYY
...
# Use first CCOLLAT record per ACCTNO for this merge (BY ACCTNO)
...
# =============================================================================
for df_ref in (overdfs_df, btrl_df, elds_df):
    if "NOTENO" not in df_ref.columns:
        df_ref = df_ref.with_columns(pl.lit(None).cast(pl.Float64).alias("NOTENO"))
...
# (EIIWCC5L groupings: matches EIIDCC5L — no '147' in '29', 131-141 in '40',
#  '111' in '50', no '123' in COTHASST routing)
# =============================================================================
CCLASSC_LOOKUP = {}

for c in ('001','006','007','014','016','024','112','113','114','115','116','117','025','026','046','048','049','149'):
    CCLASSC_LOOKUP[c] = '29'
...
for c in ('002','003','041','042','043','058','059','067','068','069','070','111','123','071','072','078','079','084','107'):
    CCLASSC_LOOKUP[c] = '90'
for c in ('004','005','127','128','129','142','143'):
    CCLASSC_LOOKUP[c] = '30'
for c in ('032','033','034','035','036','037','038','039','040','044','050','051','052','053','054','055','056','057','118','119','121','122','060','061','062'):
    CCLASSC_LOOKUP[c] = '10'
for c in ('065','066','075','076','082','083','093','094','095','096','097','098','131','132','133','134','135','136','137','138','139','141','101','102','103','104'):
    CCLASSC_LOOKUP[c] = '40'
for c in ('063','064','073','074','080','081'):
    CCLASSC_LOOKUP[c] = '60'
for c in ('010','085','086','087','088','089','090','111','125','126','144','091','092'):
    CCLASSC_LOOKUP[c] = '50'
...
# Output routing sets — matches EIIWCC5L SAS OUTPUT statements exactly
...
for r in rows:
    ...
    # Derived fields
    ficode = int(r.get("BRANCH", 0) or 0)   # FICODE = BRANCH (initial assignment)
    ...
    ficode = branch   # FICODE = BRANCH (reassigned after fallback)
    ...
    # --- COLLATER record (200 bytes, LRECL=200) ---
    rec = make_rec(200)
    ...
    if collater_code == "10":
        all_digits = bool(cprdisdt) and all(c in VALID_DIGITS for c in cprdisdt)
        if all_digits:
            continue   # VERIFY=0 → DCCMS only, skip detail file routing

    # --- Route to type-specific detail datasets ---
...
for r in cpropety_rows:
    ...
    # CPRPAR1C..4C — strip leading non-ALP character if present
    def _par(src_col):
    ...
    #    CPRPROPD IN (10,11) OR PRABANDT NE '00000000' OR
    #    CPRDISDT NE ' ' OR CPRSTAT EQ 'N' OR PAIDIND EQ 'P' OR
...
for r in cfinasst_rows:
    ...
    # Note: EIIWCC5L does NOT have the CCLASSC='147' override that X_EIBWCC5L has
...
con.close()
print("EIIWCC5L: All output files written successfully.")
