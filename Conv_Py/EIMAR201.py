...
"""
Program : EIMAR201.py
Purpose : Loans in Arrears Report for CCD PFB (Month-End Version)
          - Categorises LOANTEMP records into CAT A/B/C/D (HP Direct-Conv,
            HP 380/381, AITAB, combined HPD) based on PRODUCT + CHECKDT.
          - Merges with the branch lookup file.
          - Produces a 17-bucket arrears-aging report per branch, with
            branch subtotals and CAT grand totals.

Original JCL notes (kept for traceability):
    //DELETE   EXEC PGM=IEFBR14             -> old SASLIST (SAP.PBB.CCDTXT3)
                                                deleted; equivalent handled by
                                                opening the output file in "w"
                                                mode.
...
CHUNK_ROWS = 500_000
LRECL      = 133          # DCB LRECL=133, RECFM=FBA -> byte 0 is the ASA
                           # carriage-control character; SAS content column
                           # N (as coded in the @N clauses below) maps to
                           # buf[N], leaving buf[0] free for ASA.
...
# in the filename, same reasoning as EIMAR301.py / EIVQDCPR.py.
# ============================================================================
...
# ============================================================================
# STEP 4: BUILD LOANTEM2
# DATA LOANTEM2; FORMAT TYPE $13.; SET BNM.LOANTEMP;
...
# NOTE: four INDEPENDENT IF/OUTPUT blocks (no ELSE). A row with
# PRODUCT IN (380,381) matches CAT 'A', 'B' AND 'D' simultaneously,
# producing THREE output rows. This duplication is preserved intentionally,
# same convention as EIMAR301.py's build_loan1().
# ============================================================================
print("\nStep 4: Building LOANTEM2 (duplicate-output semantics preserved)...")
...
con.close()
gc.collect()
print(f"  LOANTEM2 rows (with duplication): {len(loantem2):,}")

# ============================================================================
# STEP 5: BUILD LOANTEMP
...
# -> equivalent to a LEFT JOIN of LOANTEM2 onto BRHDATA (unique per BRANCH),
#    keeping every LOANTEM2 row regardless of a BRHDATA match. The
#    PROC SORT BY BRANCH itself is not reproduced as a physical sort here
#    (unnecessary for the join below); the sort that materially affects
#    report output (CAT, BRANCH) is applied further down.
# ============================================================================
...
# DATA TRY; ARRAY BRHAMT{17}/NOACC{17}; SET LOANTEMP; BY CAT BRANCH;
...
# needing a manual row-by-row retained-array walk.
# ============================================================================
...
branch_universe = (
    loantemp.select(["CAT", "BRANCH", "BRHCODE"])
    .unique()
    .sort(["CAT", "BRANCH"])
)
...
def _build_header(type_label: str, pagecnt: int) -> list[str]:
    ...
    buf = _new_buf()
    _place(buf, 1, "PROGRAM-ID : EIMAR201")
    _place(buf, 43, "P U B L I C   B A N K   B E R H A D")
    _place(buf, 118, f"PAGE NO.: {pagecnt}")
    lines.append(_line(buf, "1"))

    buf = _new_buf()
    _place(buf, 35, "OUTSTANDING LOANS IN ARREARS ISSUED FROM 01 JAN 1998")
    _place(buf, 89, f"{type_label:<13s}"[:13])
