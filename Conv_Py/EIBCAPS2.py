...
"""
Program: EIBCAPS2
Purpose: Generate PBB(Staff) Movement of CAP by Branch report and build NPL.CAP_STAFF parquet
"""
...
OUTPUT1_PATH = os.path.join(OUTPUT_DIR, "EIBCAPS2.txt")
...
# ─────────────────────────────────────────────
# HP dataset (LN library, products 15,20,71,72)
# ─────────────────────────────────────────────
ln_parquet = os.path.join(LN_DIR, f"ln{REPTMON}{NOWK}{REPTYEAR}.parquet")
hp_raw = con.execute(f"SELECT * FROM read_parquet('{ln_parquet}')").pl()
...
# IND and BRANCHABBR
hp_raw = hp_raw.with_columns([
    pl.lit("PBB").alias("IND"),
    pl.col("BRANCH").map_elements(format_brchcd, return_dtype=pl.Utf8).alias("BRANCHABBR"),
])

# Filter products 15,20,71,72 with qualifying conditions
hp_df = hp_raw.filter(
    pl.col("PRODUCT").is_in([15, 20, 71, 72]) &
    (
        (pl.col("DAYARR") >= 90) |
...
# ─────────────────────────────────────────────
# SUMBAL_STAFF by CATEGORY
# ─────────────────────────────────────────────
sumbal_staff_df = (
    pbb_df.group_by("CATEGORY")
    .agg(pl.sum("BALANCE"))
)
sumbal_staff_path = os.path.join(NPL_DIR, "sumbal_staff.parquet")
sumbal_staff_df.write_parquet(sumbal_staff_path)

# ─────────────────────────────────────────────
# COUNTCAP – merge CAP1_STAFF with SUMBAL_STAFF
# Dependency: EIBCAPS1.py → produces npl/cap1_staff.parquet
# ─────────────────────────────────────────────
cap1_staff_path = os.path.join(NPL_DIR, "cap1_staff.parquet")
cap1_staff_df   = con.execute(f"SELECT * FROM read_parquet('{cap1_staff_path}')").pl()

countcap_df = (
    cap1_staff_df.join(sumbal_staff_df, on="CATEGORY", how="left", suffix="_bal")
    .with_columns([
...
# ─────────────────────────────────────────────
# NPL.CAP_STAFF{REPTMON}{REPTYEAR} – first build
# ─────────────────────────────────────────────
...
cap_staff_df.write_parquet(cap_staff_path)

# ─────────────────────────────────────────────
# GET OPENING BALANCE from year-start month
# ─────────────────────────────────────────────
cap_lmon_path = os.path.join(NPL_DIR, f"cap_staff{LMON}{LYEAR}.parquet")
...
# INDVHP merge
# ─────────────────────────────────────────────
cap_curr_df = con.execute(f"SELECT * FROM read_parquet('{cap_staff_path}')").pl()

left_keys  = set(zip(cap_curr_df["ACCTNO"].to_list(), cap_curr_df["NOTENO"].to_list()))
right_keys = set(zip(openbal_df["ACCTNO"].to_list(),  openbal_df["NOTENO"].to_list())) \
    if openbal_df.height else set()

indvhp = cap_curr_df.join(openbal_df, on=["ACCTNO", "NOTENO"], how="outer_coalesce")
...
cap_staff_final = pl.DataFrame(rows_out)
cap_staff_final.write_parquet(cap_staff_path)

# ─────────────────────────────────────────────
# Merge NPL.WMIS writeoff data (sort by ACCTNO only for staff)
# Dependency: EIBCAP42.py → produces npl/wmis.parquet
# ─────────────────────────────────────────────
wmis_npl_path = os.path.join(NPL_DIR, "wmis.parquet")

if os.path.exists(wmis_npl_path):
    woff_df = con.execute(f"SELECT * FROM read_parquet('{wmis_npl_path}')").pl()
    # IF CATEGORY='>=6 MTHS' THEN CATEGORY='6-11 MTHS'
    ...
    woff_df = pl.DataFrame(schema={
        "ACCTNO": pl.Int64, "NOTENO": pl.Int64,
        "WRIOFF_BAL": pl.Float64, "CATEGORY": pl.Utf8,
    })

cap_df2 = con.execute(f"SELECT * FROM read_parquet('{cap_staff_path}')").pl().sort("ACCTNO")

woff_acct_keys = set(woff_df["ACCTNO"].to_list()) if woff_df.height else set()
woff_map = {}
for r in woff_df.iter_rows(named=True):
    woff_map[r["ACCTNO"]] = r.get("WRIOFF_BAL", 0.0)

cap_rows = []
for row in cap_df2.iter_rows(named=True):
    acct = row["ACCTNO"]
    row["WRITEOFF"]   = "Y" if acct in woff_acct_keys else "N"
    row["WRIOFF_BAL"] = woff_map.get(acct, row.get("WRIOFF_BAL", 0.0))
    cap_rows.append(row)

cap_df2 = pl.DataFrame(cap_rows)
cap_df2.write_parquet(cap_staff_path)
...
cap_df3.write_parquet(cap_staff_path)

# ─────────────────────────────────────────────
# GENERATE SUMMARY REPORT – PBB(STAFF) MOVEMENT OF CAP BY BRANCH
# * GENERATE SUMMARY REPORT *
# TBL1 = PBB(STAFF) MOVEMENT OF CAP BY BRANCH AS AT
# ─────────────────────────────────────────────
TBL1 = "PBB(STAFF) MOVEMENT OF CAP BY BRANCH AS AT"

report_df = con.execute(f"SELECT * FROM read_parquet('{cap_staff_path}')").pl()
...
print(f"CAP_STAFF parquet  : {cap_staff_path}")
