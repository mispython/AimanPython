...
Purpose  : Read a consolidated NPL RPS report (INFIL01), detect branch header
             lines by locating the 'BRANCH :' tag at column 27, derive the CAC
           ...
           Detection method: single-line scan — BRCHTAG read from col 27-34
             of the current LINE1; BRANCH read from col 36-38 of the same line.
           NEWPAGE emits exactly ONE P001-prefixed line (LINE1 only).

           Differs from EIIMNPP5/NP6 only in column offsets:
             BRCHTAG: col 25 (NP5/NP6)  →  col 27 (NP7)
             BRANCH:  col 34 (NP5/NP6)  →  col 36 (NP7)
...
INFIL01_PATH  = os.path.join(INPUT_DIR,  "EIIMNPP7_INFIL01.txt")
OUTFIL01_PATH = os.path.join(OUTPUT_DIR, "EIIMNPP7_CAC911.txt")
OUTFIL02_PATH = os.path.join(OUTPUT_DIR, "EIIMNPP7_CAC912.txt")
OUTFIL03_PATH = os.path.join(OUTPUT_DIR, "EIIMNPP7_CAC916.txt")
OUTFIL04_PATH = os.path.join(OUTPUT_DIR, "EIIMNPP7_CAC915.txt")
OUTFIL05_PATH = os.path.join(OUTPUT_DIR, "EIIMNPP7_CAC914.txt")
OUTFIL06_PATH = os.path.join(OUTPUT_DIR, "EIIMNPP7_CAC913.txt")
OUTFIL0X_PATH = os.path.join(OUTPUT_DIR, "EIIMNPP7_COMBINED.txt")

os.makedirs(OUTPUT_DIR, exist_ok=True)

# ─────────────────────────────────────────────
# Column positions (1-based, converted to 0-based slices)
# INPUT @1 LINE1 $CHAR134. @27 BRCHTAG $8.
# INPUT @36 BRANCH $3.
# ─────────────────────────────────────────────
LINE_LEN      = 134
BRCHTAG_COL   = 27    # 1-based start of BRCHTAG  ← differs from NP5/NP6 (was 25)
BRCHTAG_WIDTH = 8
BRANCH_COL    = 36    # 1-based start of BRANCH   ← differs from NP5/NP6 (was 34)
BRANCH_WIDTH  = 3
...
#   INPUT @1 LINE1 $CHAR134. @27 BRCHTAG $8. @;   ← col 27 (NP7-specific)
#   IF BRCHTAG = 'BRANCH :' THEN DO;
#      INPUT @36 BRANCH $3.;                       ← col 36 (NP7-specific)
...
def process_infil01():
    ...
    while i < total:
        line1   = pad(lines[i])
        # @27 BRCHTAG $8.  (1-based col 27, width 8)
        brchtag = line1[BRCHTAG_START:BRCHTAG_END]
        i += 1

        if brchtag == "BRANCH :":
            # INPUT @36 BRANCH $3. – cols 36-38 of the same LINE1
