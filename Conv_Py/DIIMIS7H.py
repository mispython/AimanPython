...
Date    : 15.08.2014
SMR     : 2014-1832
Report  : ISLAMIC TERM DEPOSIT ACCOUNT (ODD DAYS TENURE)
"""
...
# MIS.DYIBUH<MM> parquet - built dynamically from reptmon
# e.g. MIS.DYIBUH06 -> DYIBUH06.parquet

# Output report file
REPORT_FILE = os.path.join(OUTPUT_DIR, 'DIIMIS7H.txt')
...
def fmt_fdfmt(mth: int) -> str:
    """
    FDFMT: numeric tenure bucket (1-12) -> day-range label.
    NOTE: DIIMIS7H uses ODD DAYS TENURE bands, not month labels.
    """
    _map = {
        1:  '30 DAYS',
        2:  '31-60 DAYS',
...
# NOTE: DIIMIS7H uses a unique 12-bucket mapping (INTPLAN 272-283 -> MTH 1-12)
#       representing ODD DAYS TENURE bands, not monthly tenures.
# ===========================================================================
...
def map_intplan_to_mth(intplan) -> int:
    """Map INTPLAN code to tenure bucket (0 if unrecognised)."""
    try:
...
def render_tabulate_old(df: pl.DataFrame, writer: ReportWriter, section_title: str):
    ...
    Replicate PROC TABULATE for OLD format (no CUSTCD dimension):
      Rows    : BRCH + TOTAL
      Columns : NO OF A/C | NO OF RECEIPT | FDI by MTH (tenure bucket) cols + TOTAL
    """
    # Collect all MTH (tenure bucket) values present (sorted)
    mth_vals   = sorted(df['MTH'].unique().to_list())
    mth_labels = [fmt_fdfmt(m) for m in mth_vals]
    col_w      = 14   # width per tenure-bucket FDI column
...
def main():
    ...
    # ------------------------------------------------------------------
    # Step 2 : Read MIS.DYIBUH<REPTMON> parquet
    # (unique to DIIMIS7H; uses DYIBUH prefix and ODD TENURE INTPLAN mapping)
    # ------------------------------------------------------------------
    dyibuh_parquet = os.path.join(INPUT_DIR, f'DYIBUH{reptmon}.parquet')

    raw_df = con.execute(
        f"SELECT * FROM read_parquet('{dyibuh_parquet}')"
    ).pl()
    ...
    with open(REPORT_FILE, 'w', encoding='utf-8') as f:
        ...
        # ==============================================================
        # OLD REPORT SECTION
        # TITLE1 'REPORT ID : DIIMIS7H (OLD)'
        # ==============================================================
        writer.set_titles(
            'REPORT ID : DIIMIS7H (OLD)',
            'PUBLIC ISLAMIC BANK BERHAD - PIBB',
            'ISLAMIC TERM DEPOSIT ACCOUNT(ODD TENURE)',
        ...
        # DATA _NULL_; SET DYIBUA1 (OBS=1) NOBS=N;
        ...
        # ==============================================================
        # NEW REPORT SECTION
        # TITLE1 'REPORT ID : DIIMIS7H (NEW)'
        # ==============================================================
        writer.new_page()
        writer.set_titles(
            'REPORT ID : DIIMIS7H (NEW)',
            'PUBLIC ISLAMIC BANK BERHAD - PIBB',
            'ISLAMIC TERM DEPOSIT ACCOUNT(ODD TENURE)',
        ...
        # NOTE: DIIMIS7H uses DATA _NULL_; SET DYIBUA1 (OBS=1) NOBS=N for both
        #       OLD and NEW sections (unlike DIIMIS7G which uses PROC CONTENTS for NEW)
