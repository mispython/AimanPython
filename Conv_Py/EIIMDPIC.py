...
"""
Program  : EIIMDPIC.py
Purpose  : Extraction for Inward Clearing Cheques from Deposits (PIBB)
           Transferred dataset to SAS Server under folder: DP_CA
           Dataset: ICLRGYYMMDD
ESMR     : 2008-1270
Date     : 09-10-08 (HHH)
Entity   : Public Islamic Bank Berhad (PIBB)
"""
...
# =============================================================================
# ICLRGD library   → SAP.PIBB.ICLRG.INPUT
ICLRGD_FILE   = BASE_OUTPUT_PATH / f"ICLRGA{REPTYEAR}{REPTMON}{REPTDAY}.parquet"
# ICLRGNEW library → SAP.PIBB.ICLRG.INPUTX  (PIBB prefix: II)
ICLRGNEW_FILE = BASE_OUTPUT_PATH / f"IICLRG{REPTYEAR}{REPTMON}{REPTDAY}.parquet"
# SAP.PIBB.DPRPT.IICLRFTP (FTP staging — PROC CPORT replacement)
ICLRGFTP_FILE = BASE_OUTPUT_PATH / f"ICLRGNEW_IICLRFTP{REPTMON}{NOWK}{REPTYEAR}.parquet"
...
# Fixed-width field layout (identical between PBB and PIBB):
...
# Fixed-width field layout (PIBB-specific field names; differs from PBB):
...
#   @028 AMOUNT    10.2   → [27:37] float (PIBB uses AMOUNT, PBB uses TRXAMT)
#   @038 MICRPRE    7.    → [37:44] num   (PIBB uses MICRPRE, PBB uses MICR)
...
#
# Note: PIBB RPTFILE uses AMOUNT and MICRPRE for positions @028 and @038,
#       whereas PBB RPTFILE uses TRXAMT and MICR for the same positions.
# -----------------------------------------------------------------------------
def parse_rptfile_pibb(filepath: Path) -> pl.DataFrame:
    """
    Read RPTFILE (CCIPS.HOSTDR.KL.CTCS) fixed-width flat file for PIBB.
    ...
    with open(filepath, "rb") as fh:
        for raw_line in fh:
            ...
            rows.append({
                ...
                "AMOUNT":  _slice_num_float(line, 28, 10, decimals=2),  # PIBB: AMOUNT
                "MICRPRE": _slice_num(line,  38,  7),                    # PIBB: MICRPRE
...
# PROC SORT BY UICCODE — applied; the SAS program explicitly PROC SORTs this
# intermediate dataset before the second merge step.
# =============================================================================
...
# =============================================================================
# DATA ICLRGUIC — parse RPTFILE (PIBB layout); filter RCRDTYPE=02; compute DRDATE
# PROC SORT BY UICCODE — removed; Polars left join does not require pre-sort.
# =============================================================================
ICLRGUIC = parse_rptfile_pibb(RPTFILE)
...
# =============================================================================
# DATA ICLRGNEW.IICLRG&REPTYEAR&REPTMON&REPTDAY — KEEP specified columns
# PIBB output dataset prefix is "II" (IICLRG...) vs PBB's "ICLRG...".
# AMOUNT and MICRPRE come from the ICLRGD side (first merge left-hand dataset)
# since those columns are already in ICLRGD after the first inner join.
# =============================================================================
...
# (SAP.PIBB.DPRPT.IICLRFTP equivalent)
# =============================================================================
