...
"""
Program  : EIVMSFLN.py
Purpose  : PIVB Staff Loan Position for HRD (Monthly) - PUBLIC INVESTMENT BANK BERHAD
...
OUTPUT_REPORT  = OUTPUT_DIR / "eivmsfln_report.txt"
...
# Note: EIVMSFLN has one label difference from EIBMSFLN:
#       product 62 = '62 RENOVATION LOAN-PIVB' (vs '62 - PIVB RENOVATION (PROG)')
# ============================================================================

def fmt_loanfmt(product) -> str:
    """
    Replicate SAS PROC FORMAT VALUE LOANFMT for EIVMSFLN (PIVB variant).
    ...
    elif p == 62:           return "62 RENOVATION LOAN-PIVB"   # PIVB variant differs here
...
#   PIVB keep filter: IF PRODUCT IN (62,70,71,72,73,74,75,76,77,78,106,107,108)
#   PRODIND Islamic: IF PRODUCT IN (106,107,108)
# ============================================================================

loan_file_bnm  = BNM_DIR  / f"loan{REPTMON}{NOWK}.parquet"
loan_file_bnmi = BNMI_DIR / f"loan{REPTMON}{NOWK}.parquet"

# IF PRODUCT IN (...): PIVB report KEEPS only these products (inverse of EIBMSFLN)
LOANTEMP_KEEP = {62, 70, 71, 72, 73, 74, 75, 76, 77, 78, 106, 107, 108}

# PRODIND Islamic classification for PIVB
PRODIND_ISLAMIC_PIVB = {106, 107, 108}
...
def build_loantemp(df: pl.DataFrame) -> pl.DataFrame:
    """
    Replicate DATA LOANTEMP for EIVMSFLN (PIVB variant):
    - RETAIN CUNDRAW: CUNDRAW = UNDRAWN if UNDRAWN > 0, else missing (None)
    - PROD = PUT(PRODUCT, LOANFMT.)
    - NOOFAC = 1
    - PRODIND: IF PRODUCT IN (106,107,108) -> 'STAFF ISLAMIC FINANCING'
               ELSE -> 'STAFF CONVENTIONAL LOAN'
    - IF PRODUCT IN (62,70,71,...): PIVB keeps only PIVB-specific products
    """
    rows = df.to_dicts()
    out  = []
    for r in rows:
        ...
        if p in PRODIND_ISLAMIC_PIVB:
            r["PRODIND"] = "STAFF ISLAMIC FINANCING"
        else:
            r["PRODIND"] = "STAFF CONVENTIONAL LOAN"

        # IF PRODUCT IN (...): PIVB report KEEPS only PIVB-specific products
        if p not in LOANTEMP_KEEP:
            continue
...
#         APPRLIMT*(N F=8.0 SUM F=COMMA18.2)
#         CURBAL  *(N F=8.0 SUM F=COMMA18.2)
#         CUNDRAW *(N F=8.0 SUM F=COMMA18.2)
#   /RTS=45 BOX='PRODUCT TYPE'   <-- NOTE: RTS=45 for PIVB (vs RTS=40 for PBB)
# ============================================================================
...
#   Identical structure to EIBMSFLN:
...
def render_report(
    ...
    """
    Replicate PROC REPORT DATA=LOANB SPLIT='*' NOWINDOWS HEADSKIP (PIVB variant).
    Structure identical to EIBMSFLN PROC REPORT.
    """
    ...
    for prod_val in product_vals:
        ...
        sub_line = (
            f" SUB TOTAL "
            f"{'NO OF A/C :':>18} {fmt_comma10(p_noofac)}"
            f"{'':>41}"
            f"{fmt_comma15_2(p_appr)}  "
            f"{fmt_comma15_2(p_undrawn)}  "
            f"{fmt_comma15_2(p_curbal)}"
        )
    ...
    grand_line = (
        f" GRAND TOTAL "
        f"{'NO OF A/C :':>16} {fmt_comma10(gt_noofac)}"
        f"{'':>41}"
        f"{fmt_comma15_2(gt_appr)}  "
        f"{fmt_comma15_2(gt_undrawn)}  "
        f"{fmt_comma15_2(gt_curbal)}"
    )
...
# --- PROC TABULATE (RTS=45 for PIVB) ---
render_tabulate(
    df=loana_by_product,
    title1="PUBLIC INVESTMENT BANK BERHAD",
    title2=(
        f"STAFF LOAN/STAFF FINANCING POSITION REPORT FOR THE MONTH OF"
        f" {WMONTH} {RYEAR}"
    ),
    footnote="REPORT ID : EIVMSFLN",
    lines=report_lines,
    rts=45,
)
