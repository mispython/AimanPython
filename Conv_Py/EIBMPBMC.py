...
"""
Program : EIBMPBMC
Purpose : Performance Report on PB Micro Finance and Impaired Loans listing.
          Incorporates conventional retail product codes 601,606,607,609 into PB Micro report.
...
OUTPUT_FILE   = os.path.join(OUTPUT_DIR, "PBMICRO.RPT.txt")

os.makedirs(OUTPUT_DIR, exist_ok=True)

PAGE_LENGTH   = 60   # lines per page (ASA default)

# Product / census filter conditions (used in multiple steps)
PRODUCT_FILTER_SQL = """
    (PRODUCT = 301 AND CENSUS IN (301.01, 320.01))
 OR (PRODUCT = 320 AND CENSUS IN (320.01))
 OR (PRODUCT = 510 AND CENSUS IN (510.01, 510.03))
 OR  PRODUCT = 574
 OR (PRODUCT = 601 AND CENSUS IN (301.01, 320.01))
 OR (PRODUCT = 609 AND CENSUS IN (320.01))
 OR (PRODUCT = 606 AND CENSUS IN (510.01, 510.03))
...
# =============================================================================
# STEP 2: Merge previous and current month loan data (LOAN<mm><wk>)
#         with product/census filters and PRODCD exclusions
# =============================================================================
prevmth_path = os.path.join(SAS_DIR, f"LOAN{prevmon}{nowk}.parquet")
curmth_path  = os.path.join(SAS_DIR, f"LOAN{reptmon}{nowk}.parquet")

# Merged dataset: LOAN<reptmon><nowk> (outer merge of prev + current by ACCTNO,NOTENO)
# WHERE PRODCD NOT IN ('34180','34240') and product/census filter
loan_merged = con.execute(f"""
    SELECT *
    FROM (
        SELECT *
        FROM read_parquet('{prevmth_path}')
    ...
    WHERE {PRODUCT_FILTER_SQL}
""").pl()

# Keep unique ACCTNO,NOTENO (outer merge behaviour -- coalesce duplicates)
loan_merged = loan_merged.unique(subset=["ACCTNO","NOTENO"], keep="last")

# =============================================================================
# STEP 3: Load DISPAY (disbursements/repayments > 0)
# =============================================================================
dispay_raw_path = os.path.join(DISPAY_DIR, f"DISPAYMTH{reptmon}.parquet")
dispay_raw = con.execute(f"""
    SELECT *
    FROM read_parquet('{dispay_raw_path}')
    WHERE DISBURSE > 0 OR REPAID > 0
""").pl()

# Inner merge LOAN_MERGED + DISPAY_RAW -> DISPAY (keep only matched)
dispay = loan_merged.join(dispay_raw, on=["ACCTNO","NOTENO"], how="inner")

# Trim DISPAY to needed columns (sorted by ACCTNO NOTENO CUSTCD FISSPURP SECTORCD)
keep_cols_dispay = ["ACCTNO","NOTENO","FISSPURP","PRODUCT","PRODCD","CUSTCD",
                    "AMTIND","SECTORCD","DISBURSE","REPAID","BRANCH","ACCTYPE"]
existing_disp = [c for c in keep_cols_dispay if c in dispay.columns]
dispay = dispay.select(existing_disp)

# =============================================================================
# STEP 4: Build CURMTH from current month file
#         (DROP=BALANCE, RENAME BAL_AFT_EIR=BALANCE)
#         Filter: PAIDIND NOT IN ('P','C') OR EIR_ADJ NE .
#         Product/census filter; BALANCE NOT NULL/0 -> NOACC=1
# =============================================================================
curmth_raw = con.execute(f"""
    SELECT * EXCLUDE (BALANCE),
           BAL_AFT_EIR AS BALANCE,
           1 AS NOACC_RAW
    FROM read_parquet('{curmth_path}')
    WHERE (PAIDIND NOT IN ('P','C') OR EIR_ADJ IS NOT NULL)
      AND ({PRODUCT_FILTER_SQL})
""").pl()

curmth = curmth_raw.with_columns(
    pl.when(~pl.col("BALANCE").is_in([None, 0]))
      .then(1)
      .otherwise(None)
      .alias("NOACC")
)
...
# =============================================================================
# STEP 5: Merge CURMTH + DISPAY -> LOAN (left join, keep all CURMTH rows)
# =============================================================================
loan = curmth.join(
    dispay.select(["ACCTNO","NOTENO","DISBURSE","REPAID"]),
    on=["ACCTNO","NOTENO"],
    how="left"
)

# =============================================================================
# STEP 6: PROC SUMMARY -- sum BALANCE, NOACC, DISBURSE, REPAID
# =============================================================================
...
# =============================================================================
# STEP 7: Write REPORT 1 -- PROC PRINT LN1 (summary totals)
# =============================================================================
rpt = ReportWriter(OUTPUT_FILE, PAGE_LENGTH)

# Titles
rpt.put('1', 'PUBLIC BANK BERHAD')
rpt.put(' ', f'PERFORMANCE REPORT ON PB MICRO FINANCE AS AT {rdate}')
rpt.put(' ', 'REPORT ID : EIBMPBMC')
rpt.put(' ', '')

# Column headers (SPLIT='*' labels)
# BALANCE='O/S BALANCE', NOACC='NO OF*ACCOUNT',
...
# SUM line (same as single data row)
rpt.put(' ', data_line)
rpt.put(' ', '')

# =============================================================================
# STEP 8: Load CCRIS data for Impaired Loans section
# =============================================================================
...
# =============================================================================
# STEP 9: Merge CURMTH + CREDMSUB + PROVSUBAC(inner) -> LN2
# =============================================================================
ln2 = (
    curmth
...
# =============================================================================
# STEP 10: Write REPORT 2 -- PROC PRINT LN2 (impaired loans detail)
# =============================================================================
rpt.put('1', 'PUBLIC BANK BERHAD')
rpt.put(' ', f'PB MICRO A/C (IMPAIRED LOANS) AS AT {rdate}')
rpt.put(' ', 'REPORT ID : EIBMPBMC')
rpt.put(' ', '')
