...
Purpose  : Monthly Report on Reasons of Closed Current & Savings Accounts
           Breakdown by Products (Conventional) as at reporting date.
           REPORT ID: EIBMCLOS
"""
...
OUTPUT_REPORT  = OUTPUT_DIR / "eibmccls_report.txt"
...
#
# DUMMYCA: I=1..8, J=1..7 -> 'B'||ZZ3(J)||ZZ2(I)
#                  K=1..9 -> 'C'||ZZ3(K)||ZZ2(I)
# DUMMYSA: I=31..42, J=1..5 -> 'B'||...
#                    K=1..8 -> 'C'||...
# DUMMYPA: J=1      -> 'P'||ZZ3(J)
# ============================================================================
...
def build_dummysa() -> list[str]:
    rows = []
    for i in range(31, 43):        # I = 31 TO 42
...
# Filter year, accumulate YC (year-to-date) and MC (current month)
...
# PROC SUMMARY (already a group-by sum — no-op at this stage)
# Merge with DUMMYCA scaffold (left join to ensure all RCOTYP present)
rca = dummyca.join(rca_raw, on="RCOTYP", how="left").with_columns([
    pl.col("YC").fill_null(0),
    pl.col("MC").fill_null(0),
])

# Derive TYPEX and RCODE, filter TYPEX IN ('01'..'08' excl '06')
rca = rca.with_columns([
    pl.col("RCOTYP").str.slice(4, 2).alias("TYPEX"),
    pl.col("RCOTYP").str.slice(0, 4).alias("RCODE"),
])
rca = rca.filter(pl.col("TYPEX").is_in(["01","02","03","04","05","07","08"]))
...
# Rename VS -> VC for reuse of accumulate_vc
savng = savng.rename({"VS": "VC"})
rsa_raw = accumulate_vc(savng, REPYY, REPMM, "YS", "MS")
rsa_raw = rsa_raw.rename({"YC": "YS", "MC": "MS"})

# Merge DUMMYSA + DUMMYPA + rsa_raw
...
# Derive TYPEX and RCODE, filter TYPEX IN ('31'..'42' relevant set)
rsa = rsa.with_columns([
    pl.col("RCOTYP").str.slice(4, 2).alias("TYPEX"),
    pl.col("RCOTYP").str.slice(0, 4).alias("RCODE"),
])
rsa = rsa.filter(
    pl.col("TYPEX").is_in(["31","32","33","34","35","38","40","41","42"])
)
# IF RCODE IN ('B009') THEN DELETE
rsa = rsa.filter(pl.col("RCODE") != "B009")
...
def render_tabulate(
    ...
    Rows = RCODE values + TOTAL row
    Cols = TYPEX values, each with MC and YC, then TOTAL MC/YC
    PRINTMISS / MISSTEXT='0': show 0 for missing cells.
    ASA '1' = form-feed, ' ' = advance one line.
    """

    # Collect all TYPEX values (sorted)
    typex_vals = sorted(df[typex_col].unique().to_list())
    # Collect all RCODE values (sorted)
    ...
    # Column headers
    # Each TYPEX has 2 sub-cols (MC, YC); last 2 cols are TOTAL MC/YC
    col_width      = 9
    rcode_width    = 35
    sep_line       = "-" * LINE_WIDTH
    ...
    def new_page() -> None:
        nonlocal line_count
        lines.append(f"1{'P U B L I C   B A N K   B E R H A D'.center(LINE_WIDTH)}")
        for t in titles:
            lines.append(f" {t.center(LINE_WIDTH)}")
        lines.append(f" {category_note.center(LINE_WIDTH)}")
        lines.append(f" ")
        lines.extend(col_header_lines())
        line_count = 5 + len(titles) + 3
...
# ============================================================================
# REPORT 1: CURRENT ACCOUNTS (CONVENTIONAL)
# ============================================================================
...
# ============================================================================
# REPORT 2: SAVINGS ACCOUNTS (CONVENTIONAL)
# ============================================================================
