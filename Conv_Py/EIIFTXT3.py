...
Purpose  : ESMR 2009-16 (SEND FILE TO LNS) - 3RD EXTRACTION
           ...
           WRITING-OFF EXERCISE for PIBB (Islamic) HP accounts.
           RIND = 'I'  (Islamic indicator)
           Sources : NPL.IIS, NPL.SP2, LOAN.LNNOTE, CISNAME.LOAN,
                     LOAN.LIAB, SASLN.LOAN{REPTMON1}{NOWKS}, NPL.LIST
           Outputs : WOFFTEXT (SAP.PIBB.FTPLNS.WOFFTXT, LRECL=750)
                     WOFFTEX1 (SAP.PIBB.WOFFTXT.DUM,    LRECL=750)
                     NPL1.WOFFTXT (permanent SAS dataset)

           Differences from EIFFTXT3:
             1. RIND = 'I'  (not 'D')
             2. JCL paths use SAP.PIBB.* (not SAP.PBB.*)
             3. ECSRIND: SAS code has bare  IF ECSRRSRV > 0 THEN ECSRIND = Y
                (Y without quotes – SAS treats unquoted Y as a variable,
                which is missing/blank, so ECSRIND stays blank when ECSRRSRV > 0
                and there is no ELSE branch).
                This behaviour is faithfully reproduced: ECSRIND = '' always.
             4. LOAN KEEP list column order differs slightly (CRRGRADE position)
                but columns are functionally identical.
...
# ─────────────────────────────────────────────
BASE_DIR    = r"C:/data"
NPL_DIR     = os.path.join(BASE_DIR, "npl")         # NPL   – SAP.PIBB.NPL.HP.SASDATA.DAILY
NPL1_DIR    = os.path.join(BASE_DIR, "npl1")        # NPL1  – SAP.PIBB.NPL.HP.SASDATA
SASLN_DIR   = os.path.join(BASE_DIR, "sasln")       # SASLN – SAP.PIBB.SASDATA
LOAN_DIR    = os.path.join(BASE_DIR, "loan")        # LOAN  – SAP.PIBB.MNILN.DAILY
CISNAME_DIR = os.path.join(BASE_DIR, "cisname")     # CISNAME – SAP.PBB.CISBEXT.LN (shared)
OUTPUT_DIR  = os.path.join(BASE_DIR, "output")

# //WOFFTEXT DD DSN=SAP.PIBB.FTPLNS.WOFFTXT  (final fixed-width output)
# //WOFFTEX1 DD DSN=SAP.PIBB.WOFFTXT.DUM     (intermediate dump)
WOFFTEXT_PATH = os.path.join(OUTPUT_DIR, "EIIFTXT3_WOFFTEXT.txt")
WOFFTEX1_PATH = os.path.join(OUTPUT_DIR, "EIIFTXT3_WOFFTEX1.txt")
NPL1_WOFFTXT  = os.path.join(NPL1_DIR,  "WOFFTXT.parquet")
PRINT_PATH    = os.path.join(OUTPUT_DIR, "EIIFTXT3_PRINT.txt")
...
#   * IIS    = ROUND(IIS,.01)       ← commented out
#   * OI     = ROUND(OI,.01)        ← commented out
#   * SP     = ROUND(SP,.01)        ← commented out
#   * TOTIIS = ROUND(IIS+OI,.01)    ← commented out
#   * TOTAL  = ROUND(TOTIIS+SP,.01) ← commented out
#   MARKETVL = ROUND(MARKETVL,.01)
#   BRNO   = SUBSTR(BRANCH,4,4)    ← Python [3:7]
#   BRABBR = SUBSTR(BRANCH,1,3)    ← Python [0:3]
# ─────────────────────────────────────────────
...
#
# NOTE: CRRGRADE appears before MARGINFI in this KEEP list (vs after NETPROC
# in EIFFTXT3). Column order in KEEP is cosmetic; logic is identical.
# ─────────────────────────────────────────────
...
def _compute(row: dict) -> dict:
    ...
    # (No ELSE branch; Y without quotes is an uninitialized SAS variable = ''.)
    # Faithful reproduction: ECSRIND is always '' regardless of ECSRRSRV value.
    ecsrind = ""
...
# LOAN KEEP list (EIIFTXT3 – CRRGRADE before MARGINFI / NETPROC ordering)
_loan_keep = [
    ...
    "GUAREND", "ISSXDTE", "NETPROC", "CRRGRADE", "MARGINFI",
...
def _fc(val, width: int) -> str:
    """$CHARw. – left-justified. Missing → spaces."""
    return f"{str(val or ''):<{width}}"[:width]

def _fn(val, width: int, dec: int = 0) -> str:
    """Numeric format w.d – right-justified. Missing → spaces."""
...
def _fd(val) -> str:
    """MMDDYY10. → 'MM/DD/YYYY'. Missing → 10 spaces."""
    return _fmt_mmddyy10(val)


# ─────────────────────────────────────────────
# Build WOFFTEX1 record (intermediate dump)
# @133 TOTAL  ← /* commented out */
# @374 CAP    ← /* commented out */
# ─────────────────────────────────────────────
def _rec_wofftex1(row: dict) -> str:
    ...
    _place(_fd(row.get("ISSXDTE")),        511,  b)
    ...
    _place(_fd(row.get("DOBMNI")),         663,  b)
...
#   TITLE2 'AS AT ' "&RDATE"
# * (trailing * after RUN in SAS is a comment – no effect)
# ─────────────────────────────────────────────
