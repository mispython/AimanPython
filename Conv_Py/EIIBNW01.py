...
def _cache_path(sas_path: Path) -> Path:
    return sas_path.with_suffix(".parquet")
...
dispay_rounded = con.execute(f"""
    SELECT
        ACCTNO, NOTENO,
        ROUND(DISBURSE, 0.01) AS DISBURSE,
        ROUND(REPAID,   0.01) AS REPAID,
        FISSPURP, PRODUCT, DNBFISME, PRODCD, CUSTCD, AMTIND, SECTORCD,
        BRANCH, ACCTYPE
...
def _derive_noacct(row: dict) -> int | None:
    """
...
def _prodesc(row: dict) -> str | None:
    product = row["PRODUCT"]
    prodcd  = row["PRODCD"]
    acctype = row["ACCTYPE"]
...
almbt_raw = con.execute(f"""
    SELECT ACCTNO, SUBACCT, FISSPURP, PRODUCT, NOTETERM, BALANCE, APPRLIM2,
...
almloan2_sme = summarize(pl.concat([almsme2, almbtcr2_src], how="diagonal"), ["PRODESC"], VAR_COLS)
almloan3     = summarize(pl.concat([dbe2, dbebt2], how="diagonal"), ["PRODESC"], VAR_COLS)
...
almprod = pl.concat([alm2_tbl.select(["TYPE", "BALANCE", "NOACCT"]),
                     almbtcr.select(["TYPE", "BALANCE", "NOACCT"])], how="diagonal")
...
# control byte. A literal form-feed marks the first line of each new page,
# matching common FB print-file convention rather than assuming ASA.)
# ============================================================================
LRECL = 133
DEFAULT_TITLE1 = "PUBLIC ISLAMIC BANK BERHAD"
DEFAULT_TITLE3 = "REPORT ID : EIIBNM01"   # literal text from the SAS source (program name mismatch preserved)
...
def _page_header(title2: str, title3: str) -> list[str]:
    return [
        DEFAULT_TITLE1,
        title2,
        title3,
        "",
        f"{'PRODESC':<35}" + "".join(f"{c:>{w}}" for c, w, _ in REPORT_COLS),
        "-" * LRECL,
    ]
...
def emit_tabulate(output_lines: list[str], rows: list[dict], class_cols: list[str],
    ...
    header_lines = [
        DEFAULT_TITLE1, title2, "",
        f"{box:<25}{'AMOUNT':>18}{'NO. OF ACCT':>14}",
        "-" * LRECL,
    ]
    output_lines.append("\f" + header_lines[0])
    output_lines.extend(header_lines[1:])
    grand_bal, grand_cnt = 0.0, 0
    for row in rows:
        label = " / ".join(str(row.get(c, "") or "") for c in class_cols)[:25]
        bal = row.get(value_col) or 0.0
        cnt = row.get(count_col) or 0
        grand_bal += bal
        grand_cnt += cnt
        output_lines.append(f"{label:<25}{_fmt_num(bal, 18, 2)}{_fmt_num(cnt, 14, 0)}")
    output_lines.append("-" * LRECL)
    output_lines.append(f"{'GRAND TOTAL':<25}{_fmt_num(grand_bal, 18, 2)}{_fmt_num(grand_cnt, 14, 0)}")
    output_lines.append("")
