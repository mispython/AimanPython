...
This program calculates specific provisions for NPL accounts based on
depreciated purchase price (20% per year depreciation). This differs from
EIFMNP06 which uses actual market values.

Key differences from EIFMNP06:
- Uses SPP1 instead of SPP2
- Uses depreciated purchase price calculation (APPVALUE - AGE * 20%)
- Different risk thresholds (91, 182, 273, 364 days)
- Filters COSTCTR < 3000 or > 3999

DISCONTINUED as per letter dated 26/08/03 from Statistics (detail report only)
"""
...
# Add parent directory to path to import format modules
sys.path.insert(0, str(Path(__file__).parent))

try:
    from PBBELF import format_brchcd
except ImportError:
    # Fallback if PBBELF not available
    def format_brchcd(branch_code):
        # Minimal implementation
        branch_map = {1: 'KLM', 34: 'PJN'}
        return branch_map.get(branch_code, f'BR{branch_code:03d}')

# Setup paths
INPUT_NPL_REPTDATE = "NPL.REPTDATE.parquet"
INPUT_NPL_IIS = "NPL.IIS.parquet"
OUTPUT_NPL_SP1 = "NPL.SP1.parquet"
OUTPUT_REPORT_SUMMARY = "NPL_SP1_SUMMARY_REPORT.txt"
OUTPUT_REPORT_DETAIL = "NPL_SP1_DETAIL_REPORT.txt"
...
def classify_risk(days):
    """
    Classify risk category based on days overdue
    Different thresholds than EIFMNP06
    """
    if days > 364:
        return "BAD"
    elif days > 273:
...
def write_summary_report(df, output_file, rdate):
    ...
    summary = df_filtered.group_by(["LOANTYP", "RISK", "BRANCH"]).agg([
    ...
    with open(output_file, 'w') as f:
        ...
        f.write(' ')
        f.write(f"{'(BASED ON PURCHASE PRICE LESS DEPRECIATION)':^132}\n")
        f.write(' \n')
        ...
        for row in summary.iter_rows(named=True):
            ...
            f.write(' ')
            risk_branch = f"  {row['RISK']:<15}{row['BRANCH']:<13}"
...
def write_detail_report(df, output_file, rdate):
    ...
    with open(output_file, 'w') as f:
        ...
        def write_header():
            ...
            f.write(' ')
            f.write(f"{'(BASED ON PURCHASE PRICE LESS DEPRECIATION)':^100}\n")
            f.write(' \n')
            f.write(' ')
...
def process_existing_npl(df_loanwoff, reptdate, styr, stmth):
    ...
    Based on depreciated purchase price
    """
    ...
    for row_dict in df_loanwoff.filter(pl.col("EXIST") == "Y").iter_rows(named=True):
        ...
        # Calculate UHC (note: uses 91 days threshold, not 89)
        if bldate is not None and bldate > datetime(1960, 1, 1).date() and termchg > 0:
            if days > 91 or borstat in ['F', 'R', 'I']:
                if issdte and reptdate:
                    remmth2 = earnterm - ((reptdate.year - issdte.year) * 12 +
        ...
        # Calculate MARKETVL (depreciated value)
        marketvl = 0.0
        appvalue = safe_float(row_dict.get('APPVALUE'))
        census7 = row_dict.get('CENSUS7', '')

        if appvalue > 0 and days > 91 and borstat not in ['F', 'R', 'I', 'Y', 'W']:
            age = int((reptdate.year - issdte.year) +
                      (reptdate.month - issdte.month) / 12) if issdte else 0
            ...
            if hardcode == 'Y':
                wrealvl = row_dict.get('WREALVL')
                if wrealvl is not None:
                    marketvl = safe_float(wrealvl)
        ...
        spp1 = safe_float(row_dict.get('SPP1'))
        sppl = sp - spp1
        ...
        risk = classify_risk(days)

        ntbrch = safe_int(row_dict.get('NTBRCH'))
        branch_code = format_brchcd(ntbrch)
        branch = f"{branch_code} {ntbrch:03d}"
...
def main():
    """
    Main processing function for EIFMNP05
    NPL Specific Provision Processing (Depreciated Purchase Price)
    """
    global REPTMON, RDATE, INPUT_NPL_LOAN

    print("EIFMNP05 - NPL Specific Provision (Depreciated Purchase Price)")
    print("=" * 70)
    print("NOTE: Detail report DISCONTINUED as per letter dated 26/08/03")
    print("=" * 70)
    ...
    # Set WDOWNIND for loan types 983, 993
    ...
    # Set EARNTERM to NOTETERM if 0 or null
    df_loanwoff = df_loanwoff.with_columns(
    ...
    print(f"Combined records (after COSTCTR filter): {len(df_loan3):,}")

    # Write output
    print(f"\nWriting output file...")
    df_loan3.write_parquet(OUTPUT_NPL_SP1)
    print(f"  {OUTPUT_NPL_SP1} ({len(df_loan3):,} records)")
    ...
    # Detail report only for specific branches (DISCONTINUED)
    print("\nGenerating detail report (KLM 001 and PJN 034 only)...")
    print("  NOTE: Detail report discontinued as per letter 26/08/03")
    ...
    if len(df_loan3) > 0:
        ...
        total_marketvl = df_loan3["MARKETVL"].sum()

        print(f"Total Current Balance: {total_curbal:,.2f}")
        print(f"Total Market Value (Depreciated): {total_marketvl:,.2f}")
    ...
    print("\nProcessing complete.")
    print("\nNote: This report uses DEPRECIATED PURCHASE PRICE")
    print("      (20% per year depreciation on APPVALUE)")
