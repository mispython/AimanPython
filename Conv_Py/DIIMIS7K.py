...
# Output report
REPORT_FILE   = os.path.join(OUTPUT_DIR, "DIIMIS7K.txt")
...
# ============================================================================
# STEP 2 – LOAD MIS.DYIBUK<MM> AND BUILD DYIBUA / DYIBUA1
# ============================================================================
...
def _parse_yyyymmdd(val) -> Optional[int]:
    ...
    if isinstance(val, (int, float)):
        s = str(int(val))
        # If 5-digit Julian Z5, convert via the year
        if len(s) == 8:
            return int(s)
        return int(val)
    if isinstance(val, datetime):
...
def load_dyibuk(ctx: dict) -> tuple[pl.DataFrame, pl.DataFrame]:
    """
    Load MIS.DYIBUK<MM>.
    ...
    """
    path = os.path.join(MIS_DIR, f"DYIBUK{ctx['reptmon']}.parquet")
    con  = duckdb.connect()
...
def _agg_by_brch_mth(df: pl.DataFrame) -> pl.DataFrame:
    """
    PROC SORT by BRANCH then PROC SUMMARY / PROC TABULATE aggregate:
    SUM of FDI, FDINO, FDINO2 grouped by BRCH × MTH.
    """
...
def _agg_by_brch_mth_custcd(df: pl.DataFrame) -> pl.DataFrame:
    """
    PROC SUMMARY aggregate: SUM of FDI, FDINO, FDINO2 grouped by BRCH × MTH × CUSTCD.
    """
    agg_cols = [c for c in ("FDI", "FDINO", "FDINO2") if c in df.columns]
    if not agg_cols:
        return pl.DataFrame()
...
# All MTH values 1–60 as column set
ALL_MTHS = list(range(1, 61))
...
def _render_tabulate(df: pl.DataFrame,
    ...
    # Sub-header: one column per MTH + TOTAL, each containing FDINO/FDINO2/FDI
    ...
    def _render_rows(subset: pl.DataFrame, row_col: str) -> dict:
        ...
        row_totals: dict = {k: {"FDINO": 0.0, "FDINO2": 0.0, "FDI": 0.0} for k in row_keys}
        mth_cell: dict   = {k: {m: {"FDINO": 0.0, "FDINO2": 0.0, "FDI": 0.0}
...
def tdbal(dyibua: pl.DataFrame, rdate: str, lines: list[str]) -> None:
    """
    %TDBAL – To-Date Balance (OLD, no CUSTCD breakdown).
    PROC SORT by BRANCH (done in load), PROC TABULATE.
    """
    agg = _agg_by_brch_mth(dyibua)
    lines.append(f"{ASA_NEWPAGE}REPORT ID : DIIMIS7K (OLD)")
    ...
    lines.append(f"{ASA_NEWLINE}(20-JUL-10 TO 15-MAY-11) AS AT {rdate}")
...
def tdbal1(dyibua: pl.DataFrame, rdate: str, lines: list[str]) -> None:
    ...
    lines.append(f"{ASA_NEWPAGE}REPORT ID : DIIMIS7K (NEW)")
    lines.append(f"{ASA_NEWLINE}PUBLIC ISLAMIC BANK BERHAD - IBU")
    lines.append(f"{ASA_NEWLINE}NEW PBB & PFB AL-MUDHARABAH FD ACCOUNT AFTER PRIVATISATION")
    lines.append(f"{ASA_NEWLINE}(20-JUL-10 TO 15-MAY-11) AS AT {rdate}")
    lines.append(f"{ASA_NEWLINE}")
...
def avg1(dyibua1: pl.DataFrame, edate_day: int, rdate: str, lines: list[str]) -> None:
    ...
    Uses PROC CONTENTS NOBS to check row count (equivalent: just check height > 0).
...
def dibmis02(ctx: dict) -> None:
    ...
    """
    dyibua, dyibua1 = load_dyibuk(ctx)
    lines: list[str] = []
...
def main() -> None:
    print("DIIMIS7K – PIBB Al-Mudharabah FD Account Report starting...")
