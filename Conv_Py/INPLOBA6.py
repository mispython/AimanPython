...
def main() -> None:
    ...
    aq_cols = ["ACCTNO", "NOTENO", "NPL", "CURBAL", "LOANTYPE"]
    ...
    nplobal_base = (
        ...
        .with_columns(
            [
                pl.lit(3).cast(pl.Int64).alias("LOANSTAT"),
            ]
        )
        .rename(
            {
    ...
    all_df = (
        ...
        sp1_df.join(sp2_df, on=["ACCTNO", "NOTENO"], how="full", suffix="_sp2")
    ...
    # nplobal_final = (
    #     all_df.join(nplobal_base, on=["ACCTNO", "NOTENO"], how="full")
    ...
    nplobal_final = (
        nplobal_base
        .join(all_df, on=["ACCTNO", "NOTENO"], how="left")
        .with_columns(pl.lit("Y").alias("EXIST"))
        .filter((pl.col("NTBRCH") > 0) & (pl.col("LOANTYPE") > 0))
        .sort(["ACCTNO", "NOTENO"])
        .unique(subset=["ACCTNO"], keep="first")
