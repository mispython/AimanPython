...
def write_report(summary: pl.DataFrame, outfile: Path) -> None:
    ...
    lines.append(" " * 40)
    lines.append(f"{sep}{' ' * 1}{sep}")
    lines.append(" " * 39 + "CURBAL")
    lines.append(f"{sep}{' ' * 1}{sep}")
    lines.append(" " * 24 + "PBB" + " " * 30 + "PIBB")
    lines.append(f"{sep}{' ' * 1}{sep}")
    lines.append(
        f"{'TYPE':<14}"
        f"{'INDIVIDUAL':<15}"
    ...
    for row in summary.sort("TYPE").iter_rows(named=True):
        ...
        line = (
            f"  {type_val}"                          # @3  (1-based → index 2)
            + " " * (15 - 2 - 6 - 2)               # pad to @15
            + f"{curbal_i:>10}"                      # @15 CURBAL_I  10.
            + " " * (30 - 15 - 10)                  # pad to @30
            + f"{curbal_c:>10}"                      # @30 CURBAL_C  10.
            + " " * (48 - 30 - 10)                  # pad to @48
            + f"{icurbal_i:>10}"                     # @48 ICURBAL_I 10.
            + " " * (62 - 48 - 10)                  # pad to @62
            + f"{icurbal_c:>10}"                     # @62 ICURBAL_C 10.
            + " " * (80 - 62 - 10)                  # pad to @80
            + f"{total:>10}"                         # @80 TOTAL     10.
        )
        lines.append(line)
    ...
    outfile.write_text("\n".join(padded) + "\n", encoding="latin1")
    print(f"[OUTFILE1] Written: {outfile}")
    print("\n".join(padded))
...
def write_delimited(summary: pl.DataFrame, outfile: Path) -> None:
    ...
    outfile.write_text("\n".join(lines) + "\n", encoding="latin1")
    print(f"[OUTFILE2] Written: {outfile}")
    print("\n".join(lines))
