...
"""
PROGRAM : EIBQADR4
PURPOSE : PRINT NAME LABELS FOR ADDR.HL (HOUSING LOAN MEMBERS) IN POSTMAIL ORDER, 3-UP LABEL FORMAT.
          INPUT : NAME.LNNAME, ADDR.HL
...
PARQUET_ADDR_HL     = INPUT_DIR / "addr_hl.parquet"

OUTPUT_FILE = OUTPUT_DIR / "EIBQADR4_labels.txt"
...
# Note: NAMELN5 is COMPRESS(NAMELN5,'') before scanning, i.e. all spaces
# removed — handled during row transformation before this function is called.
...
def main():
    ...
    # DATA HL; SET ADDR.HL; RENAME NAME=NAMELN1;
    # NOTE: BRANCH is NOT dropped here (unlike EIBQADR1/2/3) — it is kept and
    # used later for TAGLN derivation.
    hl_df = read_parquet(PARQUET_ADDR_HL)
    hl_df = hl_df.rename({"NAME": "NAMELN1"})
    # PROC SORT BY ACCTNO
    hl_df = hl_df.sort("ACCTNO")

    # DATA HL; MERGE LNAME HL(IN=A); BY ACCTNO; IF A;
    merged = hl_df.join(lname, on="ACCTNO", how="left", suffix="_LNAME")

    # Resolve NAMELN2-NAMELN8: prefer HL values; fill from LNAME if null
    for i in range(2, 9):
        col       = f"NAMELN{i}"
    ...
    # IF SUBSTR(NAMELN4,1,6)='MALAYS' THEN NAMELN4='      '  (same for 5)
    ...
    # PROC SORT DATA=HL OUT=HL; BY POSTMAIL;
