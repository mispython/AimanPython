...
Purpose : Automate the Net Stable Funding Ratio (NSFR) report for entity
          and consolidated level (PIVB) - DAILY variant.

          Combines three maturity-bucket data feeds - the mainframe GL
          feed (GLPIVB, daily), the equation-derived feed (EQUA, daily)
          and the manually maintained feed (MNL1) - summarises them by
          BNM item code, and merges the totals against a fixed positional
          item-code template (TEMPL) to produce the delimited NSFR report
          used downstream by the FTP job step.

          Identical in DATA-step logic to EIVMNSFR (monthly variant);
          only the input/output DSNs differ (DAILY vs MTHEND feeds), and
          this program has no SFTPFL command-file step (see RUNSFTP note
          near the bottom).
"""
...
OUTPUT_DIR = BASE_DIR / "output" / "EIVDNSFR"
...
# Dated DAILY feeds.
EQUA_DIR   = INPUT_DIR / "EQNSF_DAILY.txt"      # SAP.PIVB.EQNSF.DAILY.TXT(0)
GLPIVB_DIR = INPUT_DIR / "DAILY_LCR.txt"        # SAP.APPL.PIVB.DAILY.LCR(0)
# MNL1 DSN carries no DAILY/MTHEND qualifier in the original JCL - same feed
# name is shared between the monthly and daily jobs.
MNL1_DIR   = INPUT_DIR / "MNL_NSFR_LCR.txt"     # SAP.PIVB.MNL.NSFR.LCR(0)
...
# (Identical mapping to EIVMNSFR - both programs define the same local
# PROC FORMAT $NSFRCD block.)
# ============================================================================
...
# ----------------------------------------------------------------------------
# EIVDNSFR has no SFTPFL DD / no "DATA _NULL_; FILE SFTPFL;" step (unlike
# EIVMNSFR). The RUNSFTP JCL step below embeds the PUT command directly,
# using OPC scheduling substitution variables (%OYYYY., %OMM., %ODD.) rather
# than the program-computed RPTDT. Those are mainframe-scheduler variables
# resolved by OPC at submission time, not by this SAS/Python program, so
# there is no Python equivalent to generate here - left as reference only:
#
# //*%OPC SCAN
...
# lzopts linerule=$lr
# PUT //SAP.PIVB.NSFR.DAILY.TEXT(+1)   NSFR_%OYYYY.%OMM.%ODD..XLS
# EOB
# ----------------------------------------------------------------------------
...
def main() -> None:
    ...
    # (Identical formula to EIVMNSFR, even though this job is the DAILY run.)
    ...
    # SAP.PIVB.NSFR.DAILY.TEXT(+1) -- daily variant naming (DLY vs MTH).
    output_filename = f"NSFR_{rptdt}_DLY"
