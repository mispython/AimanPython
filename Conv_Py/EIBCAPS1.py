...
"""
Program: EIBCAPS1
Purpose: Generate CAP Report by Company for Staff Hire Purchase Loans
"""
...
OUTPUT1_PATH      = os.path.join(OUTPUT_DIR, "EIBCAPS1.txt")
CAP1_STAFF_PATH   = os.path.join(NPL_DIR,    "cap1_staff.parquet")
...
# ─────────────────────────────────────────────
# HP base dataset  (LN library, products 15,20,71,72)
# ─────────────────────────────────────────────
ln_parquet = os.path.join(LN_DIR, f"ln{REPTMON}{NOWK}{REPTYEAR}.parquet")
hp_df = con.execute(f"""
    SELECT *
    FROM read_parquet('{ln_parquet}')
    WHERE PRODUCT IN (15, 20, 71, 72)
      AND (DAYARR >= 90
           OR BORSTAT IN ('F','I','R')
           OR USER5 = 'N')
""").pl()
...
# (facility codes include 34371 vs HP variant)
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# Helper: build category rows and aggregate
# ─────────────────────────────────────────────
def build_group_rows(src: pl.DataFrame, rows_spec: list[dict]) -> pl.DataFrame:
    """
    For each row_spec, compute OBDEFAULT/EXPECTEDREC/CAPROVISION per source row,
    then aggregate (sum) by class keys.
    cap_formula: 'diff' = OBDEFAULT-EXPECTEDREC, 'zero' = 0
    expectedrec_formula: 'calc' = OBDEFAULT*(RECRATE/100), 'keep' = 0 (overwritten later)
    """
    frames = []
    for spec in rows_spec:
        ...
        if er_f == "calc":
            ...
            # EXPECTEDREC=EXPECTEDREC (carry-forward in SAS); initialise to 0,
            # overwritten by fix_not_repossessed_expectedrec() after aggregation
            tmp = tmp.with_columns(pl.lit(0.0).alias("EXPECTEDREC"))
...
def fix_not_repossessed_expectedrec(
    grp_df: pl.DataFrame, groupind: str, subgind: str
) -> pl.DataFrame:
    """
    Recompute EXPECTEDREC for 'NOT REPOSSESSED YET' row as sum of
    EXPECTEDREC from the three sub-categories (SUM NOT REPOSSESSED YET block).
    """
...
def assign_no_and_labels(grp_df: pl.DataFrame, mapping: dict) -> pl.DataFrame:
    ...
    for row in grp_df.iter_rows(named=True):
        ...
        if cat in mapping:
            ...
            row["SUBGROUP"] = sub
            row["NO"]       = no
            # BAL only filled when GROUP identifies the subgroup header row
            row["BAL"] = (
                f"{row['BALANCE']:,.2f}"
                if grp not in (" ", "", None) or sub not in (" ", "", None)
                else ""
            )
        else:
            row["GROUP"]    = " "
...
current_specs = [
    ...
    # *CAPROVISION=0 (commented in SAS, CAPROVISION=OBDEFAULT-EXPECTEDREC is active)
    ...
    # *CAPROVISION=OBDEFAULT-EXPECTEDREC (commented in SAS)
    ...
    # *CAPROVISION=0 (commented in SAS)
...
# SUM NOT REPOSSESSED YET
current_grp = fix_not_repossessed_expectedrec(current_grp, "OTHERS", "CURRENT")
# END SUM
current_grp = assign_no_and_labels(current_grp, {
...
# SUM NOT REPOSSESSED YET
month3t5_grp = fix_not_repossessed_expectedrec(month3t5_grp, "OTHERS", "3-5 MTHS")
# END SUM
month3t5_grp = assign_no_and_labels(month3t5_grp, {
...
# SUM NOT REPOSSESSED YET
month6t11_grp = fix_not_repossessed_expectedrec(month6t11_grp, "OTHERS", "6-11 MTHS")
# END SUM
month6t11_grp = assign_no_and_labels(month6t11_grp, {
...
# SUM NOT REPOSSESSED YET
month1t2_grp = fix_not_repossessed_expectedrec(month1t2_grp, "OTHERS", "1-2 MTHS")
# END SUM
month1t2_grp = assign_no_and_labels(month1t2_grp, {
...
repossessed_l12_grp = (
    ...
    .with_columns([
        ...
        (pl.col("BALANCE") * (1 - RECRATE / 100)).alias("CAPROVISION"),
    ])
...
# Build REPOSSESSED_GRP using skeleton merge (preserves both subgroup rows)
# SAS creates a skeleton with both subgroups then merges by SUBGROUP
repossessed_skeleton = pl.DataFrame({
    "SUBGROUP":  ["<12 MTHS ", ">=12 MTHS"],
    "GROUPIND":  ["REPOSSESSED", "REPOSSESSED"],
    "SUBGIND":   ["REPOSSESSED <12 MTHS ", "REPOSSESSED >=12 MTHS"],
})

# Align SUBGROUP key for merge
for _df in [repossessed_l12_grp, repossessed_m12_grp]:
    pass  # columns already set

rep_l12_sel = repossessed_l12_grp.drop(["GROUPIND", "SUBGIND"]) if "GROUPIND" in repossessed_l12_grp.columns else repossessed_l12_grp
rep_m12_sel = repossessed_m12_grp.drop(["GROUPIND", "SUBGIND"]) if "GROUPIND" in repossessed_m12_grp.columns else repossessed_m12_grp

# Re-add SUBGROUP for join key if not present
if "SUBGROUP" not in rep_l12_sel.columns:
    rep_l12_sel = rep_l12_sel.with_columns(pl.lit("<12 MTHS ").alias("SUBGROUP"))
if "SUBGROUP" not in rep_m12_sel.columns:
    rep_m12_sel = rep_m12_sel.with_columns(pl.lit(">=12 MTHS").alias("SUBGROUP"))

# Join skeleton with detail data
repossessed_grp = (
    repossessed_skeleton
    .join(rep_l12_sel, on="SUBGROUP", how="left")
    .join(rep_m12_sel, on="SUBGROUP", how="left", suffix="_m12")
)

# Coalesce numeric columns from the two joins
def _coalesce_col(df: pl.DataFrame, col: str) -> pl.DataFrame:
    col_m12 = col + "_m12"
    if col_m12 in df.columns:
        df = df.with_columns(
            pl.coalesce([pl.col(col), pl.col(col_m12)]).alias(col)
        ).drop(col_m12)
    return df

for _c in ["OBDEFAULT", "EXPECTEDREC", "CAPROVISION", "BALANCE", "RECRATE", "RATE"]:
    repossessed_grp = _coalesce_col(repossessed_grp, _c)
...
def _unify(df: pl.DataFrame) -> pl.DataFrame:
    needed = {
        "GROUPIND":   (pl.Utf8,    None),
        "SUBGIND":    (pl.Utf8,    None),
        "CATEGORY":   (pl.Utf8,    None),
        "RATE":       (pl.Float64, None),
        "RECRATE":    (pl.Float64, 0.0),
        "OBDEFAULT":  (pl.Float64, 0.0),
        "EXPECTEDREC":(pl.Float64, 0.0),
        "CAPROVISION":(pl.Float64, 0.0),
...
# Categories that carry null EXPECTEDREC for TOTAL aggregation
...
total_summary = (
    total_rows
    .group_by("GROUPIND")
    .agg([pl.sum("BALANCE"), pl.sum("OBDEFAULT"), pl.sum("EXPECTEDREC")])
...
# ─────────────────────────────────────────────
# Merge LAYOUT and assign fixed NOs
# ─────────────────────────────────────────────
combine = combine.join(layout_df, on="GROUPIND", how="left")

# IF GROUPIND='REPOSSESSED' THEN (sub-branch by SUBGROUP)
...
combine = combine.with_columns([
    pl.when(pl.col("GROUPIND") == "IRREGULAR").then(pl.lit(61))
      .when((pl.col("GROUPIND") == "REPOSSESSED") & (pl.col("SUBGROUP") == "<12 MTHS ")).then(pl.lit(71))
      .when((pl.col("GROUPIND") == "REPOSSESSED") & (pl.col("SUBGROUP") == ">=12 MTHS")).then(pl.lit(72))
...
# ─────────────────────────────────────────────
# PD / LGD calculations + running TOTOBDEFAULT / TOTEXPECTEDREC
# ─────────────────────────────────────────────
...
# Append SUB rows to combine and join summary columns
if subcom1.height:
...
# Compute SUBLGD / PDSUMNEW / SUBLGDNEW and TOTAL PD/LGD
combine = combine.with_columns([
...
combine = pl.DataFrame(rows_list2).sort("NO")

# ─────────────────────────────────────────────
# Helper: fixed-width line builder
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# WRITE OUTPUT REPORT
# ASA carriage control: ' ' = advance 1 line, '1' = new page
# ─────────────────────────────────────────────
...
def emit_dash(char: str = "-", count: int = 237) -> None:
    emit(" ", char * count)
...
for row in rows_final:
    ...
    # ── REPOSSESSED <12 MTHS ─────────────────────────────────────────────────
    if gi == "REPOSSESSED" and sg in ("REPOSSESSED <12 MTHS", "REPOSSESSED <12 MTHS "):
        ln = make_line(237)
...
# PROC SUMMARY CAP by SUBGIND → CAPROVISION sum → NPL.CAP1_STAFF
cap1 = (
    cap_df
    .group_by("SUBGIND")
...
cap1.write_parquet(CAP1_STAFF_PATH)

print(f"Report written      : {OUTPUT1_PATH}")
print(f"CAP1_STAFF parquet  : {CAP1_STAFF_PATH}")
