...
"""
Program : EIIDLN1M.py
Purpose : Daily Movement in Bank's Loans/OD Accounts Report (PIBB)
...
# LOAN.LOAN& / LOANX.LOAN& in the SAS source point to SAP.PIBB.SASDATA.DAILY
# → PIBB naming convention: prefix "iln" (PBB "ln" prepended with "i")
INPUT_LOAN_DIR    = BASE_DIR / "input" / "prod" / "EIIDLN1M"

# CISLN / CISDP DD statements in the original JCL point to the SAME PBB
# datasets (SAP.PBB.CISBEXT.LN / SAP.PBB.CISBEXT.DP) even in the PIBB
# version of this program — these are NOT PIBB-prefixed.
INPUT_CISLN_DIR   = Path("/stgsrcsys/host/uat") / "CISLN_loan.sas7bdat"
INPUT_CISDP_DIR   = Path("/stgsrcsys/host/uat") / "CISDP_deposit.sas7bdat"
INPUT_BRANCH_FILE = Path("/sasdata/rawdata/lookup") / "LKP_BRANCH"

# Parquet cache directory (temporary intermediates — cleared after use)
CACHE_DIR = BASE_DIR / "input" / "prod" / "EIIDLN1M"

# Output
OUTPUT_DIR  = BASE_DIR / "output" / "EIIDLN1M"
...
OUTPUT_FILE = OUTPUT_DIR / f"ILN1M{REPTDAY}{REPTMON}.txt"
...
# PIBB prefix convention: "iln" (PBB "ln" + leading "i")
# ============================================================================
print("\nStep 2: Resolving LOAN / LOANX file names...")

loan_path  = get_latest_file(INPUT_LOAN_DIR, prefix="iln")

# Derive the date embedded in the LOAN filename, then find the file whose
# encoded date is exactly one calendar day earlier.
_ln_date_match = re.search(r"iln(\d{6})", loan_path.stem, re.IGNORECASE)
...
_loanx_stem = f"iln{_loanx_date.strftime('%y%m%d')}"
...
# CISLN (~14.2 GB) — fixed filename, no date pattern (shared PBB dataset)
...
# CISDP (~1.2 GB) — fixed filename, no date pattern (shared PBB dataset)
...
#   null              → excluded (same as SAS IF CATG NE '  ')
...
def _build_header(category: str) -> list[str]:
    ...
    lines.append(f"1 PUBLIC ISLAMIC BANK BERHAD - RETAIL BANKING DIVISION")
    lines.append(f"  REPORT TITLE : EIIDLN1M")
