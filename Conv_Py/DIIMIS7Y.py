...
Date    : 23.03.10
SMR     : 2010-00000775
Report  : PIBB Al-Mudharabah FD Account, Period 16-JUN-10 until 19-JUL-10.
          Produces TWO sets of reports:
            (OLD) - standard BRCH x MTH tabulate (TDBAL + AVG)
            (NEW)  - BRCH x MTH x CUSTCD tabulate (TDBAL1 + AVG1)
          CUSTCD format: 77,78,95,96 = 'RETAIL'; OTHER = 'CORPORATE'
          REPTFQ is always 'Y'.
"""
...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, 'DIIMIS7Y.txt')
...
# ===========================================================================
# FORMAT: CDFMT  (customer code -> RETAIL / CORPORATE)
# 77,78,95,96 = 'RETAIL'; OTHER = 'CORPORATE'
# ===========================================================================
_RETAIL_CUSTCDS = {77, 78, 95, 96}

def format_cdfmt(custcd) -> str:
    if custcd is None:
        return 'CORPORATE'
    try:
        return 'RETAIL' if int(custcd) in _RETAIL_CUSTCDS else 'CORPORATE'
...
def render_fd_tabulate_old(data: pl.DataFrame, section_title: str,
                           report_titles: list[str], rdate: str) -> list[str]:
    """OLD report: BRCH x MTH -> FDINO, FDINO2, FDI + TOTAL column."""
    if data.is_empty():
        return []
...
def render_fd_tabulate_new(data: pl.DataFrame, section_title: str,
                            report_titles: list[str], rdate: str) -> list[str]:
    """
    NEW report: CUSTCD (RETAIL/CORPORATE) x BRCH x MTH -> FDINO, FDINO2, FDI.
    TABLE CUSTCD=' ', BRCH=' ' ALL='TOTAL', SUM=... * (MTH=' ' ALL='TOTAL')
    """
    if data.is_empty():
        return []

    # Map CUSTCD to RETAIL/CORPORATE label
    data = data.with_columns([
        pl.col('CUSTCD').map_elements(format_cdfmt, return_dtype=pl.Utf8).alias('CUSTCD_LABEL')
    ])

    agg = (
        data.group_by(['CUSTCD_LABEL', 'BRCH', 'MTH'])
        .agg([
            pl.col('FDINO').sum(),
            pl.col('FDINO2').sum(),
            pl.col('FDI').sum(),
        ])
        .sort(['CUSTCD_LABEL', 'BRCH', 'MTH'])
    )

    custcd_labels = sorted(agg['CUSTCD_LABEL'].unique().to_list())
    months_present = sorted(agg['MTH'].unique().to_list())
    ...
    for custcd_lbl in custcd_labels:
        lines.append(f"CUSTCD: {custcd_lbl}")
        lines.append('')

        sub_agg = agg.filter(pl.col('CUSTCD_LABEL') == custcd_lbl)
        pivot: dict[str, dict] = {}
...
def main():
    ...
    mis_parquet = MIS_DYIBUY_PARQUET_TMPL.format(reptmon=reptmon)
    ...
    # Ensure CUSTCD column exists
    if 'CUSTCD' not in df.columns:
        df = df.with_columns(pl.lit(None).cast(pl.Int64).alias('CUSTCD'))
    ...
    # DYIBUA (today): reptdate == edate
    df_today  = df.filter(pl.col('REPTDATE') == edate)
    # DYIBUA1 (period): sdate <= reptdate <= edate
    df_period = df.filter(
    ...
    # -----------------------------------------------------------------------
    old_titles = [
        'REPORT ID : DIIMIS7Y (OLD)',
        'PUBLIC ISLAMIC BANK BERHAD - IBU',
        'NEW PBB & PFB AL-MUDHARABAH FD ACCOUNT AFTER PRIVATISATION',
        f'(16-JUN-10 UNTIL 19-JUL-10) AS AT {rdate}',
    ]

    if not df_today.is_empty():
        # PROC SORT DATA=DYIBUA OUT=DYIBUAO (KEEP=BRCH MTH FDI FDINO FDINO2); BY BRANCH;
        td_lines = render_fd_tabulate_old(df_today, 'TODATE BALANCE', old_titles, rdate)
    ...
    if n > 0:
        ...
        avg_lines = render_fd_tabulate_old(sumdf, 'DAILY AVERAGE', old_titles, rdate)
    ...
    if not df_today.is_empty():
        # PROC SORT DATA=DYIBUA OUT=DYIBUAN (KEEP=BRCH MTH FDI FDINO FDINO2 CUSTCD); BY BRANCH;
        td1_lines = render_fd_tabulate_new(df_today, 'TODATE BALANCE', new_titles, rdate)
        all_lines.extend([''] + td1_lines)
    ...
    if n_new > 0:
        sumdf_new = (
            df_period.group_by(['BRCH', 'MTH', 'CUSTCD'])
            .agg([pl.col('FDI').sum(), pl.col('FDINO').sum(), pl.col('FDINO2').sum()])
        )
        days = edate.day
        sumdf_new = sumdf_new.with_columns([
            (pl.col('FDI')    / days).alias('FDI'),
            (pl.col('FDINO')  / days).alias('FDINO'),
            (pl.col('FDINO2') / days).alias('FDINO2'),
        ])
        avg1_lines = render_fd_tabulate_new(sumdf_new, 'DAILY AVERAGE', new_titles, rdate)
        all_lines.extend([''] + avg1_lines)
