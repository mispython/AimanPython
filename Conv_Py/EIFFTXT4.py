...
Purpose: ESMR 2013-705 (SEND FILE TO LNS) - 2ND EXTRACTION CAP
         Merge WOFFTXT with CAP data, write fixed-width output file,
            and produce a listing report of accounts for bad debt writing-off.
         Source: NPL (PBB) library
"""
...
NPL_DIR     = os.path.join(BASE_DIR, "npl")        # SAP.PBB.NPL.HP.SASDATA
OUTPUT_DIR  = os.path.join(BASE_DIR, "output")

os.makedirs(OUTPUT_DIR, exist_ok=True)

# SAP.PBB.FTPLNS.WOFFTXT  (WOFFTEXT DD)
WOFFTEXT_PATH = os.path.join(OUTPUT_DIR, "EIFFTXT4_wofftxt.txt")
# PROC PRINT report output
REPORT_PATH   = os.path.join(OUTPUT_DIR, "EIFFTXT4_report.txt")
...
# ─────────────────────────────────────────────
# Load NPL.CAP{REPTMON}{REPTYEAR} – deduplicate by ACCTNO+NOTENO
...
cap_df = (
    con.execute(
        f"SELECT ACCTNO, NOTENO, CAP FROM read_parquet('{cap_parquet}')"
    ).pl()
...
# ─────────────────────────────────────────────
# Merge: WOFFTXT (left) + CAP, DROP CAP from WOFFTXT before merge
...
text_df = (
    wofftxt_df
    .join(cap_df, on=["ACCTNO", "NOTENO"], how="left")
    .with_columns([
