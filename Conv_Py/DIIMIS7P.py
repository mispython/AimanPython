...
# MIS.DYIBUP<MM> parquet - built dynamically from reptmon
# e.g. MIS.DYIBUP06 -> DYIBUP06.parquet

# Output report file
REPORT_FILE = os.path.join(OUTPUT_DIR, 'DIIMIS7P.txt')
...
def main():
    ...
    # ------------------------------------------------------------------
    # Step 2 : Read MIS.DYIBUP<REPTMON> parquet
    # (differs from DIIMIS7O which reads MIS.DYIBUO<REPTMON>)
    # ------------------------------------------------------------------
    dyibup_parquet = os.path.join(INPUT_DIR, f'DYIBUP{reptmon}.parquet')

    raw_df = con.execute(
        f"SELECT * FROM read_parquet('{dyibup_parquet}')"
    ).pl()
    ...
    with open(REPORT_FILE, 'w', encoding='utf-8') as f:
        ...
        # ==============================================================
        # NEW REPORT SECTION
        # TITLE1 'REPORT ID : DIIMIS7P (NEW)'
        # ==============================================================
        writer.new_page()
        writer.set_titles(
            'REPORT ID : DIIMIS7P (NEW)',
            'PUBLIC ISLAMIC BANK BERHAD - IBU',
            'NEW PBB & PFB AL-MUDHARABAH FD ACCOUNT AFTER PRIVATISATION',
            f'(16-SEP-16 ONWARDS) AS AT {rdate}',
        )
