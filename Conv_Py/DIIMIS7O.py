...
# DYIBUO<MM> parquet - built dynamically from reptmon
# e.g. MIS.DYIBUO06 -> DYIBUO06.parquet

# Output report file
REPORT_FILE = os.path.join(OUTPUT_DIR, 'DIIMIS7O.txt')
...
def render_tabulate_old(df: pl.DataFrame, writer: ReportWriter, section_title: str):
    ...
    def _row_data(subset: pl.DataFrame):
        fdino  = subset['FDINO'].sum()
        fdino2 = subset['FDINO2'].sum()
        fdi_by_mth = {row['MTH']: row['FDI'] for row in subset.iter_rows(named=True)}
        fdi_total  = subset['FDI'].sum()
        return fdino, fdino2, fdi_by_mth, fdi_total

    # Build header lines
    col_w = 14  # width per MTH FDI column
    mth_header = ''.join(f'{lbl:>{col_w}}' for lbl in mth_labels)
    total_header = f'{"TOTAL":>{col_w}}'
    ...
    for brch in brch_vals:
        ...
        fdino, fdino2, fdi_by_mth, fdi_total = _row_data(sub)
        fdi_cols = ''.join(f'{_comma14_2(fdi_by_mth.get(m, 0)):>{col_w}}' for m in mth_vals)
        line = (
            f'{brch:<10}  '
            f'{_comma9(fdino)}  '
            f'{_comma9(fdino2)}  '
            + fdi_cols
            + f'{_comma14_2(fdi_total):>{col_w}}'
        )
    ...
    fdi_by_mth_t = (
        grp.group_by('MTH').agg(pl.sum('FDI'))
    )
    fdi_mth_map = {row['MTH']: row['FDI'] for row in fdi_by_mth_t.iter_rows(named=True)}
...
def render_tabulate_new(df: pl.DataFrame, writer: ReportWriter, section_title: str):
    ...
    for cd_label in custcd_labels:
        ...
        fdi_mth_cd = (
            sub_cd.group_by('MTH').agg(pl.sum('FDI'))
        )
...
def main():
    ...
    # ------------------------------------------------------------------
    # Step 2 : Read MIS.DYIBUO<REPTMON> parquet
    # ------------------------------------------------------------------
    dyibuo_parquet = os.path.join(INPUT_DIR, f'DYIBUO{reptmon}.parquet')

    raw_df = con.execute(
        f"SELECT * FROM read_parquet('{dyibuo_parquet}')"
    ).pl()
    ...
    with open(REPORT_FILE, 'w', encoding='utf-8') as f:
        ...
        # ==============================================================
        # OLD REPORT SECTION
        # TITLE1 'REPORT ID : DIIMIS7O (OLD)'
        # ==============================================================
        writer.set_titles(
            'REPORT ID : DIIMIS7O (OLD)',
            'PUBLIC ISLAMIC BANK BERHAD - IBU',
            'NEW PBB & PFB AL-MUDHARABAH FD ACCOUNT AFTER PRIVATISATION',
            f'(16-JUN-13 TO 15-SEP-16) AS AT {rdate}',
        )
        ...
        # ==============================================================
        # NEW REPORT SECTION
        # TITLE1 'REPORT ID : DIIMIS7O (NEW)'
        # ==============================================================
        writer.new_page()
        writer.set_titles(
            'REPORT ID : DIIMIS7O (NEW)',
            'PUBLIC ISLAMIC BANK BERHAD - IBU',
            'NEW PBB & PFB AL-MUDHARABAH FD ACCOUNT AFTER PRIVATISATION',
            f'(16-JUN-13 TO 15-SEP-16) AS AT {rdate}',
        )
