...
"""
Program : EIBMFC02
Purpose : FD by Individual and Non-Individual, by Time to Maturity for FCY Accounts
          (Weighted Average Cost by Maturity Profile)
          Uses ORIGINAL MATURITY (REMMTH = TERM) instead of remaining maturity.
Date    : 21.10.05
...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "EIBMFC02.txt")
...
def remfmt(v: float) -> str:
    """REMFMT value format for EIBMFC02 (LOW-0 = spaces)."""
    if v is None:
        return "       "
    v = float(v)
    if v <= 0:   return "       "
...
# =============================================================================
# STEP 2: Build FD expanded rows (ORIGINAL MATURITY = TERM, not REMMTH)
# =============================================================================
...
for row in fd_raw.iter_rows(named=True):
    ...
    if openind == "O" or (openind == "D" and curbal > 0):
        if openind == "D" or (matdt is not None and matdt < reptdate_dt):
            output_rows.append({
                "TYPE": type_, "PRODTYP": "FIXED DEPOSIT",
                "SUBTYP": subtyp, "SUBTTL": "B",
                "REMMTH": 99.0, "AMOUNT": curbal, "COST": curbal * rate,
            })
        else:
            remmt1 = calc_remmth(matdt, reptdate_dt) if matdt else 0.0
            # SUBTTL='A': use REMMTH = TERM (original maturity)
            output_rows.append({
                "TYPE": type_, "PRODTYP": "FIXED DEPOSIT",
                "SUBTYP": subtyp, "SUBTTL": "A",
                "REMMTH": term, "AMOUNT": curbal, "COST": curbal * rate,
            })
            # NEW FD: IF (TERM - REMMT1) < 1
            if (term - remmt1) < 1:
                rc = term - 0.5
                if   rc <= 1:  rc = 91
                elif rc <= 3:  rc = 92
                ...
                output_rows.append({
                    "TYPE": type_, "PRODTYP": "FIXED DEPOSIT",
                    "SUBTYP": subtyp, "SUBTTL": "C",
                    "REMMTH": rc, "AMOUNT": curbal, "COST": curbal * rate,
                })
...
# =============================================================================
# STEP 3: PROC SUMMARY -> FD1 (by TYPE, PRODTYP, SUBTYP, SUBTTL, REMMTH)
# =============================================================================
fd1 = (
    fd_exp
...
# =============================================================================
# STEP 4: FD2 (totals by TYPE, PRODTYP, SUBTYP); SUBTTL='Z', PRODTYP='TOTAL'
# =============================================================================
fd2 = (
    fd_exp
...
# =============================================================================
# STEP 5: Combine, compute WACOST and AMOUNT /1000
# =============================================================================
def combine_dep(d1, d2):
    all_cols = ["TYPE","PRODTYP","SUBTYP","SUBTTL","REMMTH_LBL","AMOUNT","COST"]
    def pad(df):
        for c in all_cols:
...
rpt.put(' ', "RISK MANAGEMENT REPORT : EIBMFC02")
rpt.put(' ', "RM DENOMINATION FOR FOREIGN CURRENCIES")
rpt.put(' ', "")

# Header -- SUBTYP x TYPE cross
# Columns: (USD x INDIVIDUALS), (USD x NON-INDIVIDUALS), (OTH x ...), TOTAL
hdr1 = (
    f"{'FIXED DEPOSIT':<65}"
    f"{'USD':^28}"
    f"{'OTH':^28}"
...
hdr3 = (
    f"{'':65}"
    + ("".join([f"{'BAL(RM000)':>12}{'WA.COST%':>12}" for _ in range(3)]) )
)
rpt.put(' ', hdr1)
rpt.put(' ', hdr2)
rpt.put(' ', hdr3)
...
dep_sorted = dep.sort(["PRODTYP", "SUBTTL", "REMMTH_LBL", "SUBTYP", "TYPE"])

# Pivot into wide format for each (PRODTYP, SUBTTL, REMMTH_LBL)
