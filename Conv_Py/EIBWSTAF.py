...
"""
Program  : EIBWSTAF.py
Purpose  : Weekly Listing for Staff New Loan and Paid Loan (PBB)
...
# No PBBLNFMT format functions (LNDENOM, LNPROD, CUSTCD, etc.) are called
...
STBASE_FILE     = LNHIST_DIR / "stbase.parquet"  # LNHIST.STBASE (PBB history base)

OUTPUT_DIR      = BASE_DIR / "output"
OUTPUT_REPORT   = OUTPUT_DIR / "eibwstaf_report.txt"
# PROC APPEND target — updated stbase written back
OUTPUT_STBASE   = LNHIST_DIR / "stbase.parquet"
...
# ============================================================================
# LOANTYPE FILTER for EIBWSTAF (PBB)
# WHERE (LOANTYPE LE 61 OR LOANTYPE IN(100,102,103,104,105))
#       AND COSTCTR NE 8044
# ============================================================================

def pbb_loantype_filter(loantype, costctr) -> bool:
    """Replicate PBB LNNOTE WHERE filter."""
    if loantype is None:
        return False
    lt = int(loantype)
    cc = int(costctr) if costctr is not None else 0
    return (lt <= 61 or lt in (100, 102, 103, 104, 105)) and (cc != 8044)


def pbb_iloantype_filter(loantype, costctr) -> bool:
    """Replicate PBB ILNNOTE WHERE filter (COSTCTR EQ 4043)."""
    if loantype is None:
        return False
    lt = int(loantype)
    cc = int(costctr) if costctr is not None else 0
    return (lt <= 61 or lt in (100, 102, 103, 104, 105)) and (cc == 4043)
...
def parse_sas_z11_date(val) -> date | None:
    """
    Parse SAS packed numeric date stored as Z11.:
    Format: MMDDYYYY... (first 8 chars = MMDDYYYY)
    LASTTRAN: MDY(MM=chars[0:2], DD=chars[2:4], YY=chars[4:8])
    ISSUEDT:  MDY(MM=chars[0:2], DD=chars[2:4], YY=chars[4:8])
    """
    if val is None:
        return None
    try:
...
def parse_payeffdt(val) -> str:
    """
    Replicate PAYEFF derivation from PAYEFFDT (Z11.):
    PAYEFF = chars[9:11] || '/' || chars[7:9] || '/' || chars[2:4]
    (day/month/2-digit-year string)
    """
    if val is None:
        return ""
    try:
...
def parse_freleas(val) -> date | None:
    """
    Replicate FULRELDTE = INPUT(SUBSTR(PUT(FRELEAS,Z11.),1,8), MMDDYY8.):
    First 8 chars of Z11. = MMDDYYYY
    """
...
# PREVDATE = MDY(MONTH,1,YEAR) - 1  (last day of previous month)
...
# PDATE = MDY(MMP, 1, YYP) -> first day of reporting month (SAS numeric)
PDATE_DT = date(YYP, MMP, 1)
# SDATE = MDY(MMP, SDD, YYP) -> start day of current week
SDATE_DT = date(YYP, MMP, SDD)

# SAS Z5. numeric date = days since 1960-01-01
...
# ============================================================================
# STEP 2: LOAD LNNOTE + LNCOMM (MNILN) WITH APPRLIMT DERIVATION
# ============================================================================
...
def load_and_merge_notes(
    ...
    Replicate:
      ...
      APPRLIMT derivation from COMMNO/REVOVLI/CORGAMT/CCURAMT/ORGBAL
    """
...
lnnote  = load_and_merge_notes(LNNOTE_FILE,  LNCOMM_FILE,  pbb_loantype_filter)
ilnnote = load_and_merge_notes(ILNNOTE_FILE, ILNCOMM_FILE, pbb_iloantype_filter)

# ============================================================================
# STEP 3: DATA LOAN &INTGRVAR
#   SET LNNOTE ILNNOTE;
#   Parse LASTTRAN, ISSDTE, PAYEFF, FULRELDTE from packed numeric fields
#   NOOFAC = 1
# ============================================================================
...
def build_loan(df: pl.DataFrame) -> pl.DataFrame:
    """
    Apply DATA LOAN transformations:
    - LASTTRAN: parse Z11. packed numeric -> date
    - ISSDTE:   parse Z11. packed numeric from ISSUEDT -> date
    - PAYEFF:   extract DD/MM/YY string from PAYEFFDT Z11.
    - FULRELDTE: parse FRELEAS Z11. as MMDDYYYY -> date
    - NOOFAC = 1
    - Keep only INTGRVAR columns
    """
    rows = df.to_dicts()
    out  = []
    for r in rows:
        ...
        r["NOOFAC"]    = 1
        # Keep only INTGRVAR columns (keep any extras silently)
...
#   RENAME LASTTRAN=SETTDT LSTTRNAM=SETTAMT LSTTRNCD=SETTCD
#   Filter: PAIDIND IN ('P','C')
#           DAY(LASTTRAN) GE STRDAY AND LE REPTDAY
#           MONTH(LASTTRAN) EQ REPTMTH AND YEAR(LASTTRAN) EQ REPTYEAR
#   Derive LSTRNDSC
# ============================================================================
...
for r in loan.iter_rows(named=True):
    ...
    lt_day   = lasttran.day
    lt_month = lasttran.month
    lt_year  = lasttran.year
    if not (STRDAY <= lt_day <= REPTDAY
            and lt_month == REPTMTH
            and lt_year  == REPTYEAR):
        continue
...
# ============================================================================
# STEP 5: LOAD LNHIST.STBASE (history base for PBB)
# ============================================================================

hist = pl.DataFrame()
if STBASE_FILE.exists():
    hist = con.execute(
        f"SELECT ACCTNO, NOTENO FROM read_parquet('{STBASE_FILE}')"
    ).pl().sort(["ACCTNO", "NOTENO"]).unique(subset=["ACCTNO", "NOTENO"], keep="first")

# ============================================================================
# STEP 6: DATA LNRELES + LNRELS1
#   MERGE LOAN(IN=A) HIST(IN=B); BY ACCTNO NOTENO; IF A AND NOT B
#   NMN logic: PDATE <= FULRELDTE <= EDATE & FLAG1='M' & RESTIND='M'
#   Filter: (PDATE <= ISSDTE <= EDATE) OR NMN='Y'
# ============================================================================

loan_sorted    = loan.sort(["ACCTNO", "NOTENO"])
# Mark rows present in HIST
hist_keys = set(
...
for r in loan_sorted.iter_rows(named=True):
    acctno = r.get("ACCTNO")
    noteno = r.get("NOTENO")
    # IF A AND NOT B: only rows NOT already in HIST
...
# PROC APPEND DATA=LNRELS1 BASE=LNHIST.STBASE
# Append new keys to STBASE and write back
if lnrels1_rows:
    new_keys = pl.DataFrame(lnrels1_rows)
    if not hist.is_empty():
        updated_stbase = pl.concat([hist, new_keys], how="diagonal").unique(
            subset=["ACCTNO", "NOTENO"], keep="first"
        )
    else:
        updated_stbase = new_keys
    updated_stbase.write_parquet(OUTPUT_STBASE)

# ============================================================================
# STEP 7: LNRPT1A + LNRPT1B SPLIT
#   MERGE LNSETTLE(IN=B) LNRELES(IN=A); BY ACCTNO
#   EIBWSTAF (PBB) logic:
#     IF (A AND NOT B): NMN='Y' -> LNRPT1B, else -> LNRPT1A
#     IF (A AND B):
#       IF (ORGBAL=SETTAMT) OR NMN='Y' -> LNRPT1B
#       IF (ORGBAL=SETTAMT) -> DELETE (from further output)
#       IF NID='Y' AND PAIDIND NE 'P' -> LNRPT1A
# ============================================================================
...
for r in (lnreles.iter_rows(named=True) if not lnreles.is_empty() else []):
    ...
    if not in_b:
        ...
        if (orgbal == settamt) or (nmn == "Y"):
            lnrpt1b_rows.append(r)
        if orgbal == settamt:
            pass  # DELETE from further LNRPT1A consideration
        elif nid == "Y" and paidind != "P":
            lnrpt1a_rows.append(r)
...
# ============================================================================
# STEP 8: DATA LNPAY
#   SET PAY.LNPAY&NOWK IPAY.ILNPAY&NOWK WHERE PAYAMT NE 0
#   PAYEFF = 99/MM/YY string
#   KEEP ACCTNO NOTENO PAYEFF PAYAMT
#   NODUPKEY BY ACCTNO NOTENO
# ============================================================================
...
# ============================================================================
# STEP 9: MERGE LNRPT1B WITH LNPAY -> SPLIT INTO LNRPT1B / LNRPT1C
#   MERGE LNRPT1B(IN=A) LNPAY; BY ACCTNO NOTENO; IF A;
...
if not lnrpt1b.is_empty():
    ...
    # Update PAYEFF and PAYAMT from LNPAY where available
    ...
    for r in merged_b.iter_rows(named=True):
        if r.get("NMN") == "Y":
            rpt1c_final_rows.append(r)
        else:
            rpt1b_final_rows.append(r)
    lnrpt1b = pl.DataFrame(rpt1b_final_rows) if rpt1b_final_rows else pl.DataFrame()
    lnrpt1c = pl.DataFrame(rpt1c_final_rows) if rpt1c_final_rows else pl.DataFrame()
else:
    lnrpt1c = pl.DataFrame()

# Sort final datasets for reporting
...
# ============================================================================
# REPORT 1: STAFF PAID LOAN LIST (LNSETTLE)
#   BREAK: NTBRCH -> LOANTYPE -> LSTRNDSC -> RBREAK
# ============================================================================
...
# ============================================================================
# REPORTS 2/3/4 SHARED: NEW / MIGRATION / FULL RELEASE
#   BREAK AFTER NTBRCH / LOANTYPE; RBREAK AFTER
# ============================================================================
...
def render_loan_report(
    ...
    """
    Replicate PROC REPORT for LNRPT1A / LNRPT1B / LNRPT1C:
