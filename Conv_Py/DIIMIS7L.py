...
Report  : PIBB Al-Mudharabah FD Account
          Period 16-MAY-11 Until 15-JAN-12
...
# Output report
REPORT_FILE   = os.path.join(OUTPUT_DIR, "DIIMIS7L.txt")
...
# ============================================================================
# STEP 2 - LOAD MIS.DYIBUL<MM> AND BUILD DYIBUA / DYIBUA1
# ============================================================================
...
def load_dyibul(ctx: dict) -> tuple[pl.DataFrame, pl.DataFrame]:
    """
    Load MIS.DYIBUL<MM>.
    ...
    """
    path = os.path.join(MIS_DIR, f"DYIBUL{ctx['reptmon']}.parquet")
    con  = duckdb.connect()
...
def tdbal(dyibua: pl.DataFrame, rdate: str, lines: list[str]) -> None:
    ...
    lines.append(f"{ASA_NEWPAGE}REPORT ID : DIIMIS7L (OLD)")
    ...
    lines.append(f"{ASA_NEWLINE}(16-MAY-11 UNTIL 15-JAN-12) AS AT {rdate}")
...
def tdbal1(dyibua: pl.DataFrame, rdate: str, lines: list[str]) -> None:
    ...
    lines.append(f"{ASA_NEWPAGE}REPORT ID : DIIMIS7L (NEW)")
    lines.append(f"{ASA_NEWLINE}PUBLIC ISLAMIC BANK BERHAD - IBU")
    lines.append(f"{ASA_NEWLINE}NEW PBB & PFB AL-MUDHARABAH FD ACCOUNT AFTER PRIVATISATION")
    lines.append(f"{ASA_NEWLINE}(16-MAY-11 UNTIL 15-JAN-12) AS AT {rdate}")
    lines.append(f"{ASA_NEWLINE}")
...
def dibmis02(ctx: dict) -> None:
    ...
    """
    dyibua, dyibua1 = load_dyibul(ctx)
    lines: list[str] = []
...
def main() -> None:
    print("DIIMIS7L - PIBB Al-Mudharabah FD Account Report starting...")
