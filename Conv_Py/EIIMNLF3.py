...
Assumptions:
- Source input datasets are available as parquet files with SAS variable names.
- Output report is text and includes ASA carriage control characters.
...
NLF_REPORT_PATH = OUTPUT_DIR / "SAP_PIBB_NLF_TEXT.txt"
...
PRODUCT_TO_ITEM_AND_TEXT = {
    "9321109": ("A1.01", "- FIXED TERM FINANCING  "),
    "9521109": ("A1.01", "- FIXED TERM FINANCING  "),
    "9321209": ("A1.02", "- REVOLVING FINANCING  "),
    "9521209": ("A1.02", "- REVOLVING FINANCING  "),
    "9321309": ("A1.03", "- CASH LINE FACILITY"),
    "9521309": ("A1.03", "- CASH LINE FACILITY"),
    "9321909": ("A1.04", "- OTHERS"),
    "9521909": ("A1.04", "- OTHERS"),
    "9321408": ("A1.05", "- HOUSING FINANCING"),
    "9521408": ("A1.05", "- HOUSING FINANCING"),
    # "9321508": ("A1.06", "- CREDIT CARDS"),
    # "9521508": ("A1.06", "- CREDIT CARDS"),
    "9321308": ("A1.07", "- CASH LINE FACILITY"),
    "9521308": ("A1.07", "- CASH LINE FACILITY"),
    ...
    "9421109": ("B1.01", "- FIXED TERM FINANCING  "),
    "9621109": ("B1.01", "- FIXED TERM FINANCING  "),
    "9421209": ("B1.02", "- REVOLVING FINANCING  "),
    "9621209": ("B1.02", "- REVOLVING FINANCING  "),
    "9421309": ("B1.03", "- CASH LINE FACILITY"),
    "9621309": ("B1.03", "- CASH LINE FACILITY"),
    "9421909": ("B1.04", "- OTHERS"),
    "9621909": ("B1.04", "- OTHERS"),
    "9421408": ("B1.05", "- HOUSING FINANCING"),
    "9621408": ("B1.05", "- HOUSING FINANCING"),
    # "9421508": ("B1.06", "- CREDIT CARDS"),
    # "9621508": ("B1.06", "- CREDIT CARDS"),
    "9421308": ("B1.07", "- CASH LINE FACILITY"),
    "9621308": ("B1.07", "- CASH LINE FACILITY"),
...
def build_store_table(finalsum_df: pl.DataFrame) -> pl.DataFrame:
    """Build STORE dataset equivalent after DATA + PROC TRANSPOSE."""
    loan = (
        finalsum_df.with_columns(
            ...
            ((pl.col("AMOUNT").cast(pl.Float64) / 1000.0).round(0) * 1000.0).alias("AMOUNT"),
        )
        .with_columns(pl.col("PROD").replace(PRODUCT_TO_DESC, default=None).alias("DESC"))
    )

    transposed = (
        loan.with_columns(pl.int_range(1, pl.len() + 1).over(["PROD", "DESC"]).alias("_SEQ"))
...
def build_report_slice(store_df: pl.DataFrame, product_prefix: str) -> pl.DataFrame:
    """Build REPORT dataset for one section."""
    return (
        store_df.filter(
...
def write_section(handle, title: str, section_df: pl.DataFrame, page_length: int = DEFAULT_PAGE_LENGTH) -> None:
    """Write one report section with ASA carriage-control characters."""
    lines_on_page = 0
